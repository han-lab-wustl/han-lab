function loadVideoTiffNoSplit_EH2_new_sbx_uint16Opto3(varargin)
...
stims = [];
%temps = [];
stripeTempN = zeros(432, 56, numframes, 'uint16');

%%
for ii=1:ceil(numframes/lenVid)
    % for ii=1:4
    stripeTemp = sbxread(stripped_filename,((ii-1)*lenVid),min(lenVid,(numframes-((ii-1)*lenVid))));
    %stripeTemp = sbxreadPartial(stripped_filename,((ii-1)*lenVid)+1,min(lenVid,(numframes-((ii-1)*lenVid))), 1:20, 1:20);
    stripeTemp = double(squeeze(stripeTemp));
    stripeTemp= stripeTemp(81:512,741:796,:);
    stripeTempN(:,:,(ii-1)*3000+1:min(ii*3000,numframes)) = stripeTemp;
    %temps =  [temps squeeze(nanmean(squeeze(nanmean(stripeTemp))))];
end

temps = squeeze(nanmean(squeeze(nanmean(stripeTempN))));
...
%% For the exponetial fit 
   ...
   filimage = repmat(filimage, [1, 1, size(chtemp,3)]);
   chtemp = chtemp-filimage;
         end
