...
optoEventsFullLength = zeros(size(VR.imageSync));
for i = 1:length(optoEventsIdx)
    idx = optoEventsIdx(i);
    optoEventsFullLength(idx:idx + opto_frames - 1) = 1;
end
...
urew_solenoidALL=[]; rewardsALL=[]; solenoid2ALL=[];  rew_us_solenoidALL=[];  forwardvelALL=[];  ybinnedALL=[];
 trialnumALL=[]; changeRewLocALL=[]; licksALL=[];  lickVoltageALL=[]; optoEventsALL = [];
for newindx = 1:length(utimedFF)
    if newindx == 1
        ...
        optoEventsALL(newindx) = sum(optoEventsFullLength(find(uVRtimebinned<=after))); % New addition
    elseif newindx == length(utimedFF)
        ...
        optoEventsALL(newindx) = sum(optoEventsFullLength(find(uVRtimebinned>before))); % New addition
    else
        before = mean([utimedFF(newindx) utimedFF(newindx-1)]);
        after = mean([utimedFF(newindx) utimedFF(newindx+1)]);
        ...
        elseif isempty(find(uVRtimebinned>before & uVRtimebinned<=after)) && after > check_imaging_start_before
            ...
            optoEventsALL(newindx) = sum(optoEventsFullLength(find(uVRtimebinned>before & uVRtimebinned<=after))); % New addition
...
for n = 1:numfiles
    ...
    save(fullFfile,'ybinnedALL','rewardsALL','forwardvelALL','licksALL','changeRewLocALL','trialnumALL','utimedFF','lickVoltageALL','solenoid2ALL','urew_solenoidALL','rew_us_solenoidALL','optoEventsALL','-append');
    load(fullFfile);
    
    clear ybinned rewards forwardvel licks changeRewLoc trialnum  timedFF lickVoltage solenoid2 urew_solenoid rew_us_solenoid 
