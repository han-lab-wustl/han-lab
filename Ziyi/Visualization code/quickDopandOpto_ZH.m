% Ziyi Change to NaN out the planes when opto stimulation is on 
clear all
close all
pr_dir=uipickfiles; 
 ...
 for days=days_check
    ...
    planefolders = dir_s2p(:,~cellfun(@isempty,regexp(dir_s2p(1,:),'plane')));
     ...
     for allplanes=1:size(planefolders,2) %1:4
         ...
         %hotfix for weird things
         abfrect(cellfun(@length,abfrect)<50) = [];

        stimsPlanes = deinterleave_array(stims, 4);
        stims_single_plane = stimsPlanes(:,allplanes);

        dFF = params.roibasemean3{1};
        dFF_base_mean=mean(dFF);
        dFF = dFF/dFF_base_mean;
        %dFF(find(stims_single_plane)) = NaN;
        ...
        deleteidx(find(deleteidx)+1) = 1;
        if length(deleteidx)>length(nonCS)
            deleteidx(end) = [];
        end
        nonCS([deleteidx]) = [];
        for x = 1:length(xs)
       ...
       deleteidx = [];
        ...
        title(['Peri Rand Opto Plane ' num2str(allplanes)])
            subplot(2,4,allplanes*2)
             for x = 1:length(xs)
            randCS = randi(length(availableidx)-1);
            randtemp = FakeCS(availableidx(randCS):availableidx(randCS)+1);
            randxs = FakeCS(availableidx(randCS))+8+rand*(diff(randtemp)-16);
            availableidx(randCS) = [];
            currx = find(timedFF>=(randxs-timewindow)&timedFF<(randxs+timewindow));
            yyaxis left
            plot(timedFF(currx)-randxs,dFF(currx),'-','Color',planecolors{allplanes})
        ...
        title(['Peri Rand nonOpto Plane ' num2str(allplanes)])
        end
