...
for allcat=1%1:11%1:11%1:11%1:11
    ...
    workspaces = {'223Alldays_dark_reward_workspace_03.mat','224Alldays_dark_reward_workspace_03.mat','225Alldays_dark_reward_workspace_02.mat'};
...
%                 files=1:size(roi_dop_alldays_planes_perireward,1);
   ...
   %% 
    ...
    subplot(2,2,1), s=scatter(Grabda_idx(1:6),Grabda(1:6),'ro','filled');
    hold on
    scatter(Grabda_idx(7:end),Grabda(7:end),[],[0.5 0.5 0.5],'filled');
    set(gca,'xlim',[0 3]); hold on;
    plot([1 2],mean(Grabda_mean),'ks')
    errorbar(mean(Grabda_mean),std(Grabda_mean)/sqrt( size(Grabda_mean,1)),'k-','Linewidth',2,'Capsize',0);
...
%     s.MarkerFaceAlpha =0.4;
    ...
    subplot(2,2,2), s=scatter(Grabdamut_idx(1:3),Grabdamut(1:3),[],[0.5 0 0],'filled');
    hold on
    scatter(Grabdamut_idx(4:end),Grabdamut(4:end),[],[0.3 0.3 0.3],'filled');
    set(gca,'xlim',[0 3]); hold on;
    plot([1 2],mean(Grabdamut_mean),'ks')
    errorbar(mean(Grabdamut_mean),std(Grabdamut_mean)/sqrt( size(Grabdamut_mean,1)),'k-', 'Linewidth',2,'Capsize',0);
