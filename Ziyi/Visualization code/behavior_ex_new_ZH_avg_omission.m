...
for alldays = 1:length(pr_dir0)
    ...
    for allplanes = 1:4
        ...
        if exist(pr_dir, 'dir')
            ...
            cs_loc = find(solenoid2);
            us_loc = find(rewards==1);
            %CSUStimelag = 0.5;
            framerate = 7.8;
            threshold = ceil(CSUStimelag*framerate);

            result = []; % Initialize result with the first element

            for i = 2:length(cs_loc)
                if cs_loc(i) - cs_loc(i-1) > threshold
                    result(end+1) = cs_loc(i-1); % Append if difference is greater than or equal to threshold
                end
            end

            % Initialize an array to store unmatched values
            rew_omission_idx = [];

            % Loop through each element in result
            for i = 1:length(result)
                % Check if any element in us_loc is within 4 to 5 units of the current element in result
                if all(abs(result(i) - us_loc) > 5)
                    % If no element in us_loc is within 4-5 units, add it to unmatched array
                    rew_omission_idx = [rew_omission_idx, result(i)];
                end
            end
