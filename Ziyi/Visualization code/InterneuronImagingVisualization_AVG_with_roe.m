...
for alldays = 1:length(pr_dir0)%
    ...
    for allplanes = 1:4 %%change plane info
        ...
        if exist(pr_dir, 'dir')
            ...
            speed_smth_1=smoothdata(speed_binned,'gaussian',5)';

            if mod(length(speed_smth_1), 4) ~= 0
                error('Data length must be divisible by 4.');
            end

            % Reshape the data into a 4-row matrix
            reshaped_data = reshape(speed_smth_1, 4, []);
            ...
            %all_plane_dff{1,allplanes} = roi_event_data;
...
mean_speed = mean(speed_event_data,1);
...
for p = 1:4
    ...
    ylabel('dF/F')
    mean_data = mean(all_plane_dff{p}, 1);
    norm_mean_data = mean_data./mean(mean_data(1:pre_win_frames));
    yax = norm_mean_data;
    se_yax=std(all_plane_dff{p},[],1)./sqrt(size(all_plane_dff{p},1))';
