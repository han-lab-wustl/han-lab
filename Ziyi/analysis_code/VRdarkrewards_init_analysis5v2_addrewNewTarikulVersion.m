...
for alldays = 1:length(pr_dir0)%[3:12 14:19]%[3:12 13:19]%[3 5:1]%[5:12 14]%[5:12 14]%1:5%26:30%1:33%31:32%30%27:30%21%[1:21]%[1:2 4:5 12:22]%[8:21]%[2:4 6:11]%[1:2 4:5 12:20]%%[1:21]%%
    ...
    for allplanes=1:4
        ...
        if exist( pr_dir, 'dir')
            ...
            speedftol=10; % number of frames above cm/s that we can still merge two stops together
...
%                 savefig(currfile)
                ...
                for i=1:length(singlerew)
                    ...
                    if currentnrrewUSidxperplane+post_win_frames<=length(roibase_mean)
