...
%     path='G:\dark_reward\dark_reward_allmouse_3';%% WITH EARLIEST DAYS
        path='E:\Ziyi\processed_data\HRZ_workspaces'
...
%         '171_dark_reward_workspace' '170_dark_reward_workspace.mat','179_dark_reward_workspace.mat','181_dark_reward_workspace.mat'};
         ...
         workspaces = {'156_HRZ3_workspace.mat','157_HRZ2_workspace.mat','167_HRZ2_workspace_00.mat','168_HRZ2_workspace.mat','169_HRZ2_workspace.mat',...
            '220_HRZ_workspace.mat','221_HRZ_workspace.mat','222_HRZ_workspace.mat',...
            '171_HRZ_workspace' '170_HRZ2_workspace.mat' '179_HRZ2_workspace.mat','158_HRZ_workspace.mat','010_HRZ2_workspace.mat','011_HRZ2_workspace.mat'};
    
        mutnames = {'170','179','158','010','011'};
        mutindxss = find(cellfun(@(x) sum(ismember(x(1:3),mutnames))>0,workspaces,'Un',1));
    ...
    for rr= 2:length(workspaces)
        ROI_labels{rr} = {'Plane 1 SR','Plane 2 SP','Plane 3 SO','Plane 4 SO'};
    end
    % 170
    ROI_labels{cellfun(@(x) strcmp(x(1:3),'170'),workspaces,'Un',1)} = {'Plane 1 SR','Plane 2 SR_SP','Plane 2 SP','Plane 3 SP','Plane 3 SP_SO', 'Plane 4 SO'};
...
%         withoutSO_allmouse_mut_roe=[out3; out1];
        ...
        for ll = 1:length(workspaces)
            ...
            if length(intersect(ll,mutindxss))>0
...
figure;
subplot(4,2,3)
scatter(ones(size(SOslopes(setxor(1:length(workspaces),mutindxss)))),SOslopes(setxor(1:length(workspaces),mutindxss)),25,'r','filled','Jitter','on','Jitteramount',0.04)
hold on
scatter(ones(size(SOslopes(mutindxss)))*2,SOslopes(mutindxss),25,'r','filled','Jitter','on','Jitteramount',0.04)

 plot([1 2],[nanmean(SOslopes(setxor(1:length(workspaces),mutindxss))) nanmean(SOslopes(mutindxss))],'ks')
    errorbar([1 2],[nanmean(SOslopes(setxor(1:length(workspaces),mutindxss))) nanmean(SOslopes(mutindxss))],[nanstd(SOslopes(setxor(1:length(workspaces),mutindxss)))/sqrt(length(SOslopes(setxor(1:length(workspaces),mutindxss)))) nanstd(SOslopes(mutindxss))/sqrt(length(SOslopes(mutindxss)))],'k','Capsize',0,'LineStyle','none','LineWidth',1.5)
   xlim([0.5 2.5])
   plot([0.5 2.5],[0 0],'k--')
   ...
   ylim([-4 2]*10^-3)
   yticks((-4:2:2)*10^-3)
%    [~,p] = ttest2(SOslopes(setxor(1:length(workspaces),mutindxss)),SOslopes(mutindxss));
%    text(1.5,nanmean(SOslopes),['p:' num2str(p)])
    [p] = signrank(SOslopes(setxor(1:length(workspaces),mutindxss)));
    text(1,nanmean(SOslopes(setxor(1:length(workspaces),mutindxss))),['p:' num2str(p)])
    [p] = signrank(SOslopes(mutindxss));
    text(2,nanmean(SOslopes(mutindxss)),['p:' num2str(p)])
   
   
 subplot(4,2,4)

scatter(ones(size(notSOslopes(setxor(1:length(workspaces),mutindxss)))),notSOslopes(setxor(1:length(workspaces),mutindxss)),25,[0.5 0.5 0.5],'filled','Jitter','on','Jitteramount',0.04)
hold on

scatter(ones(size(notSOslopes(mutindxss)))*2,notSOslopes(mutindxss),25,[0.5 0.5 0.5],'filled','Jitter','on','Jitteramount',0.04)
 plot([1 2],[nanmean(notSOslopes(setxor(1:length(workspaces),mutindxss))) nanmean(notSOslopes(mutindxss))],'ks')
    errorbar([1 2],[nanmean(notSOslopes(setxor(1:length(workspaces),mutindxss))) nanmean(notSOslopes(mutindxss))],[nanstd(notSOslopes(setxor(1:length(workspaces),mutindxss)))/sqrt(length(SOslopes(setxor(1:length(workspaces),mutindxss)))) nanstd(notSOslopes(mutindxss))/sqrt(length(SOslopes(mutindxss)))],'k','Capsize',0,'LineStyle','none','LineWidth',1.5)
   xlim([0.5 2.5])
   plot([0.5 2.5],[0 0],'k--')
   ylabel('Linear Fit Slope')
   xticks(1:2)
  title('notSO')
   ...
   ylim([-4 2]*10^-3)
   yticks((-4:2:2)*10^-3)
%    [~,p] = ttest2(notSOslopes(setxor(1:length(workspaces),mutindxss)),notSOslopes(mutindxss));
%    text(1.5,nanmean(notSOslopes),['p:' num2str(p)])
[p] = signrank(notSOslopes(setxor(1:length(workspaces),mutindxss)));
    text(1,nanmean(notSOslopes(setxor(1:length(workspaces),mutindxss))),['p:' num2str(p)])
    [p] = signrank(notSOslopes(mutindxss));
    text(2,nanmean(notSOslopes(mutindxss)),['p:' num2str(p)])
   
%
% figure;
subplot(4,2,7)
scatter(ones(size(SORsquared(setxor(1:length(workspaces),mutindxss)))),SORsquared(setxor(1:length(workspaces),mutindxss)),25,'r','filled','Jitter','on','Jitteramount',0.04)
hold on
scatter(ones(size(SORsquared(mutindxss)))*2,SORsquared(mutindxss),25,'r','filled','Jitter','on','Jitteramount',0.04)

 plot([1 2],[nanmean(SORsquared(setxor(1:length(workspaces),mutindxss))) nanmean(SORsquared(mutindxss))],'ks')
    errorbar([1 2],[nanmean(SORsquared(setxor(1:length(workspaces),mutindxss))) nanmean(SORsquared(mutindxss))],[nanstd(SORsquared(setxor(1:length(workspaces),mutindxss)))/sqrt(length(SORsquared(setxor(1:length(workspaces),mutindxss)))) nanstd(SORsquared(mutindxss))/sqrt(length(SORsquared(mutindxss)))],'k','Capsize',0,'LineStyle','none','LineWidth',1.5)
   xlim([0.5 2.5])
   plot([0.5 2.5],[0 0],'k--')
   ylabel('rsquared')
   xticks(1:2)
...
%    ylim([-4 2]*10^-3)
%    yticks((-4:2:2)*10^-3)
%    [~,p] = ttest2(SORsquared(setxor(1:length(workspaces),mutindxss)),SORsquared(mutindxss));
%    text(1.5,nanmean(SORsquared),['p:' num2str(p)])
   
   
 subplot(4,2,8)

scatter(ones(size(notSORsquared(setxor(1:length(workspaces),mutindxss)))),notSORsquared(setxor(1:length(workspaces),mutindxss)),25,[0.5 0.5 0.5],'filled','Jitter','on','Jitteramount',0.04)
hold on

scatter(ones(size(notSORsquared(mutindxss)))*2,notSORsquared(mutindxss),25,[0.5 0.5 0.5],'filled','Jitter','on','Jitteramount',0.04)
 plot([1 2],[nanmean(notSORsquared(setxor(1:length(workspaces),mutindxss))) nanmean(notSORsquared(mutindxss))],'ks')
    errorbar([1 2],[nanmean(notSORsquared(setxor(1:length(workspaces),mutindxss))) nanmean(notSORsquared(mutindxss))],[nanstd(notSORsquared(setxor(1:length(workspaces),mutindxss)))/sqrt(length(SORsquared(setxor(1:length(workspaces),mutindxss)))) nanstd(notSORsquared(mutindxss))/sqrt(length(SORsquared(mutindxss)))],'k','Capsize',0,'LineStyle','none','LineWidth',1.5)
   xlim([0.5 2.5])
   plot([0.5 2.5],[0 0],'k--')
   ylabel('Rsqaured')
   xticks(1:2)
...
%    [~,p] = ttest2(notSORsquared(setxor(1:length(workspaces),mutindxss)),notSORsquared(mutindxss));
...
figure;
subplot(4,2,3)
scatter(ones(size(SOloccor(setxor(1:length(workspaces),mutindxss)))),SOloccor(setxor(1:length(workspaces),mutindxss)),25,'r','filled','Jitter','on','Jitteramount',0.04)
hold on
scatter(ones(size(notSOloccor(setxor(1:length(workspaces),mutindxss))))*2,notSOloccor(setxor(1:length(workspaces),mutindxss)),25,[0.5 0.5 0.5],'filled','Jitter','on','Jitteramount',0.04)


 plot([1 2],[nanmean(SOloccor(setxor(1:length(workspaces),mutindxss))) nanmean(notSOloccor(setxor(1:length(workspaces),mutindxss)))],'ks')
    errorbar([1 2],[nanmean(SOloccor(setxor(1:length(workspaces),mutindxss))) nanmean(notSOloccor(setxor(1:length(workspaces),mutindxss)))],[nanstd(SOloccor(setxor(1:length(workspaces),mutindxss)))/sqrt(length(SOloccor(setxor(1:length(workspaces),mutindxss)))) nanstd(notSOloccor(setxor(1:length(workspaces),mutindxss)))/sqrt(length(SOloccor(setxor(1:length(workspaces),mutindxss))))],'k','Capsize',0,'LineStyle','none','LineWidth',1.5)
   xlim([0.5 2.5])
   plot([0.5 2.5],[0 0],'k--')
...
%    ylim([-4 2]*10^-3)
%    yticks((-4:2:2)*10^-3)
%    [~,p] = ttest2(SOloccor(setxor(1:length(workspaces),mutindxss)),SOloccor(mutindxss));
%    text(1.5,nanmean(SOloccor),['p:' num2str(p)])
    [~,p] = ttest(SOloccor(setxor(1:length(workspaces),mutindxss)));
    text(1,nanmean(SOloccor(setxor(1:length(workspaces),mutindxss))),['p:' num2str(p)])
    [~,p] = ttest(notSOloccor(setxor(1:length(workspaces),mutindxss)));
    text(2,nanmean(notSOloccor(setxor(1:length(workspaces),mutindxss))),['p:' num2str(p)])
   
   
 subplot(4,2,4)

scatter(ones(size(SOloccor(mutindxss))),SOloccor(mutindxss),25,'r','filled','Jitter','on','Jitteramount',0.04)
hold on

scatter(ones(size(notSOloccor(mutindxss)))*2,notSOloccor(mutindxss),25,[0.5 0.5 0.5],'filled','Jitter','on','Jitteramount',0.04)
 plot([1 2],[nanmean(SOloccor(mutindxss)) nanmean(notSOloccor(mutindxss))],'ks')
    errorbar([1 2],[nanmean(SOloccor(mutindxss)) nanmean(notSOloccor(mutindxss))],[nanstd(SOloccor(mutindxss))/sqrt(length(SOloccor(mutindxss))) nanstd(notSOloccor(mutindxss))/sqrt(length(SOloccor(mutindxss)))],'k','Capsize',0,'LineStyle','none','LineWidth',1.5)
   xlim([0.5 2.5])
   plot([0.5 2.5],[0 0],'k--')
   ylabel('Linear Fit Slope')
   xticks(1:2)
...
%    ylim([-4 2]*10^-3)
%    yticks((-4:2:2)*10^-3)
%    [~,p] = ttest2(notSOloccor(setxor(1:length(workspaces),mutindxss)),notSOloccor(mutindxss));
%    text(1.5,nanmean(notSOloccor),['p:' num2str(p)])
[~,p] = ttest(SOloccor(mutindxss));
    text(1,nanmean(SOloccor(mutindxss)),['p:' num2str(p)])
    [~,p] = ttest(notSOloccor(mutindxss));
    text(2,nanmean(notSOloccor(mutindxss)),['p:' num2str(p)])
   ylim([-1 1])
   yticks(1:0.5:1)
