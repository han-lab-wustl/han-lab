...
optoEvents_full = zeros(size(VR.imageSync));
for i = 1:length(optoEventsIdx)
    idx = optoEventsIdx(i);
    optoEvents_full(idx:idx + opto_frames - 1) = 1;
end

%cuts all of the variables from VR
optoEventsALL = optoEvents_full(scanstart:scanstop);
...
urew_solenoidALL=[]; rewardsALL=[]; solenoid2ALL=[];  rew_us_solenoidALL=[];  forwardvelALL=[];  ybinnedALL=[];
 trialnumALL=[]; changeRewLocALL=[]; licksALL=[];  lickVoltageALL=[];
