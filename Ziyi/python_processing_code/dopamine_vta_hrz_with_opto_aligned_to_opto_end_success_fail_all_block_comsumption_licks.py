...
Aligned to the end of opto, the end would vary across trials 
...
for day in days: 
    ...
    for path in Path(os.path.join(src, str(day))).rglob('params.mat'):
        ...
        # remove comsumption licks
        '''
        # Initialize the result array
        rewlocsArr = np.zeros(eps[-1])  # Size 11250
        rewlocsNew = rewlocs - 10
        
        # Fill segments
        for i in range(len(rewlocsNew)):
            rewlocsArr[eps[i]:eps[i+1]] = rewlocsNew[i]
        
        block_mask = ybinned > rewlocsArr
        lick_rate[block_mask] = np.nan
        '''
    ...
    # Construct trial category dictionaries using your variable names
        ...
        if pln == 0:
            ...
            plt.savefig(os.path.join(save_dir, f"D{days[0]}_behavior.png"), dpi=300)
            #pdf.savefig(fig)
