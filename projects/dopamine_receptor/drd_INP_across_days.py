...
for ii,mouse_name in enumerate(mice):
    ...
    for dy in days:
        ...
        # Construct the search pattern for files that begin with 'plane' and end with 'all_days.mat'
        pattern = os.path.join(day_dir, '**', 'plane*all_days.mat')

        # Recursively search for files that match the pattern
        file_list = glob.glob(pattern, recursive=True)
        # sort for planes
        file_list.sort()
        # Check if any files were found and display results
        if not file_list:
            print('No files found that match the criteria.')
        else:
            for plane, file in enumerate(file_list):
                f = loadmat(file)
                print(file)
