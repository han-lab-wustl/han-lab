...
%                 end
                ...
                [handles.files(idx).rois_ibar,handles.files(idx).centroids,handles.files(1).roi_areas,handles.files(idx).iscell] ...
 = load_S2P_rois(dat,handles.files(idx).dims);
                ...
                handles.files(idx).roi_status = [ones(size(handles.files(idx).rois_ibar,1),1), ...%zd added comma
                    zeros(size(handles.files(idx).rois_ibar,1),1)];
...
% Check if 'stat' is a cell array and extract the 'iscell' field
if iscell(dat.stat)
    iscell_values = cellfun(@(x) x.iscell, dat.stat);
else
    iscell_values = [dat.stat.iscell];
end

% Identify the indices of cells
s2p_iscell = find(iscell_values);

    % Create zero matrix with the accumulated total elements and 4 columns
rois_ibar = zeros(sum(cellfun(@(x) numel(x.ypix), {dat.stat{s2p_iscell}}')), 4);
...
for r = 1:numel(s2p_iscell)
    ...
    tmwa=(s2p_iscell(r) + (0 * these_idcs));
    rois_ibar(prev_id:prev_id+numel(these_idcs)-1,:) = [these_idcs' border' area'  tmwa']; % Added 20170511 HD to fix ROI indexing
