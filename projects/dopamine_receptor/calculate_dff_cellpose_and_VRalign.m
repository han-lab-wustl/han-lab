...
for dy=days
    ...
    for plane = 1:length(Fmat)
        f =load(fullfile(Fmat(plane).folder, Fmat(plane).name));
        clls = size(f.dFF_iscell,1);
        statiscell = f.stat(logical(f.iscell(:,1)));
        % remove merged rois that stay in mat
        garbage = zeros(1,20);
        ii=1;
        for st=1:length(statiscell)
            rois = statiscell{st}.imerge;
            if length(rois)>0
                garbage(ii:ii+length(rois)-1)=rois;
                ii=ii+length(rois);
            end
        end
        ...
        % indices
        cllsind = 1:size(f.F,1);
        cllsindiscell = cllsind(logical(f.iscell(:,1)));
        keepind = ~ismember(cllsindiscell,garbage);
        f.dFF_iscell = f.dFF_iscell(keepind,:);
        subpl = ceil(sqrt(size(f.dFF_iscell,1)));
        clls = clls-sum(~keepind);
        figure;        
        for cll=1:clls% plot per cell in separate subplot                       
            subplot(subpl,subpl,cll); 
            plot(f.dFF_iscell(cll,:)); 
            title(string(cll))
        end
