...
def readposedata(dfpth,fall):
    """_summary_

    Args:
        dfpth (_type_): _description_
        fall (_type_): _description_

    Returns:
        _type_: _description_
    """
    df = pd.read_csv(dfpth) #r'Y:\DLC\DLC_networks\pupil_licks_nose_paw-Zahra-2023-02-27\videos\230215_E201DLC_resnet50_pupil_licks_nose_pawFeb27shuffle1_50000.csv'
    mat = loadmat(fall) # load fall with behavior aligned data
    # r'Z:\sstcre_imaging\e201\2\230215_ZD_000_001\suite2p\plane0\Fall.mat'
    ...
    dfkmeans = pd.DataFrame(np.array([[os.path.basename(dfpth)]*len(forwardvelocity),
        blinks[1::2], #binning by taking every other value, is this the right way???
        nose[1::2],tongue[1::2],
        mouth_open[1::2], forwardvelocity]).T)
    columns = ['mouse', 'blinks','nose','tongue','mouth_open', 'velocity']
    dfkmeans.columns=columns

    return dfkmeans

mouse_data = ['230301_e201', '230304_e201', '230307_e201', '230215_e201'] # eye video names
fmats = r'Z:\sstcre_imaging\e201'
days = listdir(fmats)
flds = [listdir(dy, ifstring="ZD") for dy in days]
flds = [fld[0] for fld in flds if fld != []]
modelsrc = r'Y:\DLC\DLC_networks\pupil_licks_nose_paw-Zahra-2023-02-27\videos'
dfs = []
for mouse in mouse_data:
    dfpth = [xx for xx in listdir(modelsrc) if ".csv" in xx and mouse.upper() in xx and "shuffle3" in xx][0]
    fallfld = [xx for xx in flds if mouse[:6] in xx][0]
    fall = os.path.join(fallfld, "suite2p", "plane0", "Fall.mat")
    dfs.append(readposedata(dfpth, fall))
...
dfkmeans = pd.concat(dfs)
dfkmeans.to_csv(r'C:\Users\Han\Desktop\test.csv')
datacol = ['blinks','nose','tongue','mouth_open', 'velocity']
dfkmeans[datacol]=dfkmeans[['blinks','nose','tongue','mouth_open', 'velocity']].astype("float32")
plt.figure()
plt.plot(dfkmeans['blinks'])
plt.axhline(y=dfkmeans["blinks"].mean()+dfkmeans["blinks"].std()*4, 
        color='r', linestyle='-')
plt.figure()
plt.plot(dfkmeans['nose'].values)
plt.axhline(y=dfkmeans["nose"].mean()+dfkmeans["nose"].std()*4, 
        color='r', linestyle='-')

plt.figure()
plt.plot(dfkmeans['mouth_open'])
plt.axhline(y=dfkmeans["mouth_open"].mean()+dfkmeans["mouth_open"].std()*4, 
            color='r', linestyle='-')
...
X_scaled=StandardScaler().fit_transform(dfkmeans[datacol])#,'mouth_open1','mouth_open2']])
...
#convert to df...
X_scaled = pd.DataFrame(X_scaled, columns=datacol)#,'mouth_open1','mouth_open2'])
