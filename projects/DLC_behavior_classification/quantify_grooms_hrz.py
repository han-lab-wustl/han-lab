...
def get_long_grooms_per_ep(dlcfls,row):
    ...
    if 'HRZ' in mat['experiment'] and sum(mat['rewards']>0) and len(eps)>2: # only for hrz
        ...
        if sum(paw_y.values)>0:
            ...
            if len(starts)>0:
                ...
                beforerewgrs, afterrewgrs, darktimegrs,rewzgrs = categorize_grooming(eps, mat,starts,rewz)
                if hrz_summary:    
...
def categorize_grooming(eps, mat,starts,rewz):
    # categories for periods of grooming
    rewzgrs = []; darktimegrs = []
    ...
    return beforerewgrs, afterrewgrs, darktimegrs, rewzgrs
