...
plt.close('all')
# save to pdf
condrewloc = pd.read_csv(r"C:\Users\Han\Downloads\data_organization_halo_control_yfp_grab.csv", index_col = None)
src = r"Y:\yfp_grabda"
animals = ['e244','e245','e246']#,'e242','e243']
days_all = [[66, 67, 68, 69, 70, 71, 72],
            [40,41,42,43,67,68,69,70,71,72,73,74,75],
            [43,44,45,46,47,48,49,50,51]]#,[29,30],[36,37]]
...
plt.savefig(os.path.join(dst, 'memory_lick_selectivity_ctrl_opploc.svg'), bbox_inches='tight')
...
# lick_selectivity_near_rewardloc_mean
plt.figure(figsize=(2.2,5))
ax = sns.barplot(x='opto_day_before', y='success_rate', 
                hue='opto_day_before', data=df, fill=False,errorbar='se',
                palette={False: "slategray", True: "crimson"})
ax = sns.stripplot(x='opto_day_before', y='success_rate', 
                hue='opto_day_before', data=df,
                palette={False: "slategray", True: "crimson"},
                s=12, alpha=0.3
                )
sns.stripplot(x='opto_day_before', y='success_rate', 
                hue='opto_day_before', data=dfagg,
                palette={False: "slategray", True: "crimson"},
                s=15,ax=ax)
...
x1 = df.loc[(df.opto==1), 'success_rate'].values
x2 = df.loc[df.opto==False, 'success_rate'].values
t,pval1 = scipy.stats.ranksums(x1[~np.isnan(x1)], x2[~np.isnan(x2)])
dfagg = df.groupby(['animal', 'opto_day_before']).mean(numeric_only = True)
...
ax.spines[['top','right']].set_visible(False)
plt.title(f'persession: {pvals1:.4f}\n paired t-test: {pvals2:.4f}',fontsize=12)
plt.savefig(os.path.join(dst, 'nextday_performance_ctrl_opploc.svg'), bbox_inches='tight')

#%%
# lick selectivity last 8 trials
# df=df[df.licks_selectivity_last8trials>.2]
...
ax.spines[['top','right']].set_visible(False)
ax.get_legend().set_visible(False)
fig.suptitle('Current Day')
ax.set_xlabel('')

plt.savefig(os.path.join(dst, 'currect_day_lick_sel_ctrl_opploc.svg'), bbox_inches='tight')
