...
days_all = [[72,73,74,75,76,77,78,79,80,81],
            [64,65,66,67,68,69,70,71,72,73],[73,74,75,76,77,78,79,80,81,82]]
...
lds = [[condrewloc.loc[((condrewloc.Day.values==dy)&(condrewloc.Animal.values==animals[ii])), 'learning_day'].values[0] for dy in dys] 
...
ax.spines[['top','right']].set_visible(False)
ax.get_legend().set_visible(False)
plt.savefig(os.path.join(dst, 'fewrewmostlystim_online_performance_halo.svg'), bbox_inches='tight')
#%%
# success rate
...
ax.get_legend().set_visible(False)
ax.spines[['top','right']].set_visible(False)
plt.savefig(os.path.join(dst, 'fewrewmostlystim_performance_success_rate_halo.svg'), bbox_inches='tight')