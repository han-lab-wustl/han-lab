...
%             end
       ...
       numplanes=size(planefolders,2);
           ...
           dop_success_peristop=[]; roe_success_peristop=[];
                ...
                %%%%% moving
                find_figure(strcat(string(mouse_id),'_perilocomotion moving','_plane',num2str(allplanes)));
                ...
                subplot(ceil(sqrt(length(pr_dir0))),ceil(sqrt(length(pr_dir0))),cnt),
                plot(frame_time*(-pre_win_frames)*numplanes:frame_time*numplanes:frame_time*post_win_frames*numplanes, ...
                    mean(dop_success_perimov,1),'k','LineWidth',2);
                ...
                plot(frame_time*(-pre_win_frames)*numplanes:frame_time:frame_time*post_win_frames*numplanes, ...
                    rescale(mean(roe_success_perimov,1),0.985,0.99),'k--','LineWidth',2)
            ...
            for roii = 1:size(df_f,1)
                ...
                stim_idx = find(stims);
                                    ...
                                    lick_success_peristim = zeros(pre_win_framesALL+post_win_framesALL+1, ...
                                        length(stim_idx));
                ...
                save('params','roinorm_stim_traces','roistim_traces', ...
                    'roe_success_peristim','lick_success_peristim','stim_idx','-append');
                    %% SAVING SECTION
