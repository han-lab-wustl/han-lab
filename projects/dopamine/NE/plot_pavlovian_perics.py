...
for ii,animal in enumerate(animals):
    ...
    for day in days: 
        ...
        for path in list(Path(os.path.join(src, animal, str(day))).rglob('params.mat')):
            ...
            dff = np.hstack(dffdf.rolling(7).mean().values)
            # find stops
            ...
            ax.plot(cs-1)
            ax.set_ylim([-.1,.1])
            ax.set_title(f'CS events')
            # peri stop binned activity
            normmeanrewdFF, meanrewdFF, normrewdFF, \
                rewdFF= eye.perireward_binned_activity(dff, cs, 
            ...
            ax.set_title(f'Peri-CS (NE)')
            ax.set_ylabel('$\Delta$ F/F')
            
            # peri stop velocity
            normmeanrewdFF, meanv, normrewdFF, \
                rewv = eye.perireward_binned_activity(params['forwardvel'][0], cs, 
                    timedFF, range_val, binsize)
