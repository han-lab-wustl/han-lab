...
%for zd folder structure
[pth,mousenm,~]=fileparts(fileparts(pr_dir0{1}));   

for alldays = 1:length(pr_dir0)%%%number of days to run data on
    [pth,daynm,~]=fileparts(pr_dir0{alldays}); %for zd folder structure
    clearvars lickVoltage forwardvel    
    planeroicount = 0;
    cnt=cnt+1;
    Day=alldays;
    for allplanes=1:3 %4%%%number of planes to feed
        plane=allplanes;
        % ZD changed to make modular based on new scanbox software
        pr_dir1 = dir(fullfile(pr_dir0{Day},'**\suite2p'));
        pr_dir=fullfile(pr_dir1(1).folder,sprintf('plane%i', plane-1),'reg_tif');
        ...
        if exist( pr_dir, 'dir')
            ...
            legend({'Speed','Dopamine','Reward'})
            ylabel('dF/F')
            title(sprintf('%s, day %s, plane %i', mousenm, daynm, plane))
...
%                 plot(rescale(supraLick,-10,-5),'Color',[0.7 0.7 0.7])
                ...
                save('params','didntstoprew','rew_stop_success_tmpts', ...
                    'nonrew_stop_success_tmpts','stop_success_tmpts','mov_success_tmpts','-append')
                ...
                plot(smoothdata(forwardvelALL,'gaussian',round(gauss_win*1.5)).*rew_idx2,'bo')
                title(sprintf('%s, day %s, plane %i', mousenm, daynm, plane))
                ylabel('Smoothed forward velocity at reward')
                ...
                save('params','roe_success_peristop_no_reward', ...
                    'dop_success_peristop_no_reward','roe_success_peristop_reward', ...
                    'dop_success_peristop_reward','dop_success_perimov','dop_success_peristop', ...
                    'roe_success_perimov','roe_success_peristop','-append')
                ...
                %%%%% moving
                find_figure(strcat(mousenm,' perilocomotion moving',' plane',num2str(allplanes)));

                subplot(ceil(sqrt(length(pr_dir0))),ceil(sqrt(length(pr_dir0))),cnt),
                % individual traces
                plot(frame_time*(-pre_win_frames)*numplanes:frame_time*numplanes:frame_time*post_win_frames*numplanes, ...
                    dop_success_perimov);
                hold on
                % mean trace
                plot(frame_time*(-pre_win_frames)*numplanes:frame_time*numplanes:frame_time*post_win_frames*numplanes, ...
                    mean(dop_success_perimov,1), ...
                    'k','LineWidth',3);
                ylabel('dF/F')
                legend(['n = ',num2str(size(dop_success_perimov,1))])
                hold on
                % zd rescaled because dff does not go from 0 to 1??
                plot(frame_time*(-pre_win_frames)*numplanes:frame_time:frame_time*post_win_frames*numplanes, ...
                    rescale(mean(roe_success_perimov, 1),min(mean(dop_success_perimov,1))-5, ...
                    max(mean(dop_success_perimov,1))+5),'k--', 'LineWidth',1)
                legend(['velocity'])  
...
%                 savefig(['ROI_' num2str(roii) 'Fl_Speed_Reward.fig'])
                ...
                title({sprintf('%s, day %s, plane %i', mousenm, daynm, plane)},{strcat(['ROI' num2str(roii) 'PeriReward Fl single reward'])})
...
%                 savefig(currfile)
                ...
                title({sprintf('%s, day %s, plane %i', mousenm, daynm, plane)},{strcat(['ROI' num2str(roii) 'PeriReward Fl rewCS'])})
...
%                 savefig(currfile)
                ...
                title({sprintf('%s, day %s, plane %i', mousenm, daynm, plane)},{strcat(['ROI' num2str(roii) 'PeriReward Fl single rewUS'])})
...
%                 savefig(currfile)
                ...
                title({sprintf('%s, day %s, plane %i', mousenm, daynm, plane)},{strcat(['ROI' num2str(roii) 'non rewarded licks'])})
...
%                 savefig(currfile)
                ...
                if exist('urew_solenoidALL','var')
                    ...
                    title({sprintf('%s, day %s, plane %i', mousenm, daynm, plane)},{strcat(['ROI' num2str(roii) 'PeriReward Fl single unrewarded CS'])})
...
%                     savefig(currfile)
                ...
                if exist('rew_us_solenoidALL','var')
                    if length(find(rew_us_solenoidALL))>0
                        ...
                        ylabel('dF/F')
                        plot(frame_time*(-pre_win_frames)*numplanes:frame_time*numplanes:frame_time*post_win_frames*numplanes, ...
                            roinorm_rew_us_single_tracesUS);%auto color
                        plot(frame_time*(-pre_win_frames)*numplanes:frame_time*numplanes:frame_time*post_win_frames*numplanes, ...
                            mean(roinorm_rew_us_single_tracesUS,2),'k','LineWidth',2);
                        legend(['n = ',num2str(size(roinorm_rew_us_single_tracesUS,2))])%n=
                        plot(frame_time*(-pre_win_frames)*numplanes:frame_time:frame_time*post_win_frames*numplanes, ...
                            rescale(mean(roinorm_rew_us_single_traces_roesmthUS,2),0.99,0.995),'k','LineWidth',2);
                        title({sprintf('%s, day %s, plane %i', mousenm, daynm, plane)},{strcat(['ROI' num2str(roii) 'PeriReward Fl single USonly US'])})
...
%                         savefig(currfile)
                ...
                if exist('singlerew_us','var')
                    if ~isempty(singlerew_us)
                        ...
                        title({sprintf('%s, day %s, plane %i', mousenm, daynm, plane)},{'Single rewards US only'})
...
%                         savefig(currfile)
                        ...
                        title({sprintf('%s, day %s, plane %i', mousenm, daynm, plane)},{ ...
                            strcat(['ROI' num2str(roii) ' PeriReward Fl drew missedUS double rew CS.fig'])})
...
%                         savefig(currfile)
                ...
                legend(['n = ',num2str(size(roinorm_double_traces,2))])%n=
                 title({sprintf('%s, day %s, plane %i', mousenm, daynm, plane)},{ ...
                            strcat(['ROI' num2str(roii) ' PeriReward Fl double rewards.fig'])})
...
%                 savefig(currfile)
                ...
                if exist('double_idx','var')
                    ...
                    title({sprintf('%s, day %s, plane %i', mousenm, daynm, plane)},{ ...
                            strcat(['ROI' num2str(roii) ' PeriReward Fl double reward CS.fig'])}) 
...
%                     savefig(currfile)
                    ...
                    legend(['n = ',num2str(size(roinorm_double_tracesCS,2))])%n=
                    title({sprintf('%s, day %s, plane %i', mousenm, daynm, plane)},{ ...
                            strcat(['ROI' num2str(roii) ' PeriReward Fl double reward US.fig'])}) 
                ...
                if exist('roisingle_traces','var')
                    ...
                    for stamps=1:length(stop_success_tmpts)
                        ...
                        if currentidx+post_win_frames<length(roinorm_base_mean)
                            try % zd added for when start ind is 0?
                                roidop_success_peristop(stamps,:)= roinorm_base_mean(currentidx-pre_win_frames:currentidx+post_win_frames);
                            catch 
                                iind = currentidx-pre_win_frames+1;
                                if iind==0
                                    roidop_success_peristop(stamps,:)= roinorm_base_mean(iind:currentidx+post_win_frames);
                                end
                            end
                        end
                    end
...
%                    
                    ...
                    legend({'vr speed', 'moving middle', 'stop', 'reward', ...
                        'stops', 'non-rewarded stops', 'rewarded stops', ...
                        'lick during non-rewarded stops', ...
                        'non-rewarded and non-licking stops', 'licks'})   
                    title({sprintf('%s, day %s, plane %i', mousenm, daynm, plane)}) 
