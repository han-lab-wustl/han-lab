...
%                 files=1:size(roi_dop_alldays_planes_perireward,1);
    ...
    for currmouse = 1:length(workspaces)
        ...
        for jj = 1:size(dopvariable,2)
            ...
            roe_sig_alldays=imresize(squeeze(roevariable(files,1,:)),[size(dop_sig_alldays,1) 79]);
            ...
            roe_sig_early=imresize(squeeze(roevariable(files(earlydays),1,:)),[1 79]);
            roe_sig_late=imresize(squeeze(roevariable(files(fliplr(length(files)-earlydays+1)),1,:)),[1 79]);
