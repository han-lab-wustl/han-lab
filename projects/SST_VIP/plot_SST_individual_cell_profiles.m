...
load("Y:\sstcre_analysis\fmats\e135\e135_day001_plane0_Fall.mat");
pln0 = load("Y:\sstcre_analysis\fmats\e135\e135_day001_plane0_Fall.mat");
pln1 = load("Y:\sstcre_analysis\fmats\e135\e135_day001_plane1_Fall.mat");
pln2 = load("Y:\sstcre_analysis\fmats\e135\e135_day001_plane2_Fall.mat");
plns = {};
plns{1} = pln0; plns{2} = pln1; plns{3} = pln2;
planes = 3;
bin_size = 6; %cm, lower causes missing vals
...
for ep = 1:length(eps)-1 % for each epoch
    ...
    for pln=1:planes % for each plane
        ...
        for cll=1:size(dff,1) % mean trial plot each cell
            subplot(ceil(sqrt(size(dff,1))),ceil(sqrt(size(dff,1))),cll)
            plot(meandff(:,cll), 'k'); hold on
            xline(rewloc/bin_size, 'b--')
            ylabel('dFF')
            xticks(0:10:nbins)
            xticklabels(0:60:track_length)
            yyaxis right
            plot(vel_bin, 'Color', purple); 
