...
%remove them only from the regfile!!
...
for p=1:planes
    ...
    [names{p},paths{p}]=uigetfile('*.mat',char(strcat("pick your Template Day's files. Plane ",num2str(p))));
end

for p=1:planes
    disp(['Plane ', num2str(p)]);
    [regnames{p},regpaths{p}]=uigetfile('*.mat',char(strcat("pick your combined reg files. Plane ", num2str(p))));
end
...
for p=1:planes
    ...
    load([regpaths{p},regnames{p}])
    %removing cells as per reg2p
    dFF = dFF(:,iscell(:,size(iscell,2)));
    F = F(:,iscell(:,size(iscell,2)));
    nF = nF(:,iscell(:,size(iscell,2)));
    masks = masks((iscell(:,size(iscell,2))),:,:);
    Fc = Fc(:,iscell(:,size(iscell,2)));
    %%%
    ...
    masks(lose,:,:)=[];
    Fc(:,lose)=[];
%     save([paths{p},names{p}],'F','dFF','nF','masks','Fc','frame','-append');
    mask{p}=masks;
    dFFp{p}=dFF;
    Fraw{p}=F;
    iscell(lose,:) = [];
    save([regpaths{p},regnames{p}],'iscell','-append');
    
    if exist('frame','var')
        frames{p}=frame;
    else
        frames{p}=zeros(size(mask{p},2),size(mask{p},3));
    end
    ...
    for p2=p:planes
        ...
        if ~isempty(p_centroids{p}) && ~isempty(p_centroids{p2})
            ...
            if ~isempty(likely_overlap)
                ...
                %compare individual cell traces to determine if they're the
                %same cells
...
%% save results
for p=1:planes
    load([paths{p},names{p}],'F','dFF','nF','masks','Fc','frame');
    load([regpaths{p},regnames{p}])
    ...
    nF(:,elim{p})=[];
    notUsed=elim{p};
    %     save([paths{p},names{p}],'F','dFF','nF','masks','notUsed','Fc','frame','-append');
    iscell(elim{p},:) = [];
    save([regpaths{p},regnames{p}],'iscell','-append');
    % Added by SH on 12/27/2019 to see the final cells with mean image
    % after elimination of same cells over planes
    temp = squeeze(sum(masks,1));
    figure;
    colormap('gray')
    subplot(1,2,1)
    imagesc(temp)
    subplot(1,2,2)
    imagesc(frame)
    saveas(gcf,[regpaths{p},'post_plane_elim_masks.jpg'])
end
%%
