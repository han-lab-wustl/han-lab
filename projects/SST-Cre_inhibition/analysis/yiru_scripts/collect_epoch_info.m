clearvars
Settings.paths=dir('Z:\sstcre_imaging\e201\*\Fall.mat');
Settings.level_mouse_name = 3;
Settings.level_day = 4;
ata = struct(); % if want to create a new dataset, comment line 6 and lines 14-16 (if ismember(day_cd ......)
% load('Y:\E186\E186\alldays_info.mat'); % contains a struct called data
rewrange = 10 *2/3;
for day = 1:size(Settings.paths,1)
    file = fullfile(Settings.paths(day).folder,Settings.paths(day).name);
    directory = file;
    ...
    load(file)
    specific_date = extractAfter(extractBefore(VR.name_date_vr, "_time"), "E186_");
    data(day,1).day = day_cd;
    data(day,1).date = char(datetime(specific_date,'InputFormat','dd_MMM_yyyy','Format','yyyy-MM-dd'));
    data(day,1).mouse = mouse_cd;
    ...
    for ep = 1:length(rewloc)
        ...
        data(day,1).day_eps(ep,1).epoch = ep;
        data(day,1).day_eps(ep,1).RewLoc = rewl;
        data(day,1).day_eps(ep,1).success_info = success;
        data(day,1).day_eps(ep,1).opto_stim = opto;
        data(day,1).day_eps(ep,1).licks = licksnum;
        
        data(day,1).day_eps(ep,1).mean_speed = mean_speed;
        data(day,1).day_eps(ep,1).mean_speed_no_stop = mean_speed_no_stop;
        data(day,1).day_eps(ep,1).mean_speed_one_third_track = mean_speed_one_third;
        data(day,1).day_eps(ep,1).mean_speed_in_dark = mean_speed_dark;

        data(day,1).day_eps(ep,1).probe_trial = probe;
        data(day,1).day_eps(ep,1).probe_licks = pb_licks;
        data(day,1).day_eps(ep,1).probe_opto = pb_opto;
...
save('Y:\E186\E186\alldays_info.mat','data')
disp('All done!')