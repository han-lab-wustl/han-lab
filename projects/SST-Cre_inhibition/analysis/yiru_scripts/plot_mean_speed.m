...
for this_day = 1:size(data,1)
    ...
    figure('visible','off','Renderer','painters','Position', [20 20 1000 700])
    t = tiledlayout('flow');
    xt = [];
    lg = strings(1,length([eps.epoch])-1);
    opto = [-1 -1];
    nexttile
    for i = 1:length([eps.epoch])-1
        os = eps(i).opto_stim;
        linestyle = '-';
        if sum(os)>0
            opto = find(os == 1);
    ...
    xlabel('Trial number')
    ylabel('Speed (cm/s)')
    title(['Mean speed of trials on ' char(data(this_day).date) '(' char(data(this_day).day) ')'])
    hold off
    ...
    xlabel('Trial number')
    ylabel('Speed (cm/s)')
    title(['Mean speed (nonstop) of trials on ' char(data(this_day).date) '(' char(data(this_day).day) ')'])
    hold off
    ...
    xlabel('Trial number')
    ylabel('Speed (cm/s)')
    title(['Mean speed (in dark) of trials on ' char(data(this_day).date) '(' char(data(this_day).day) ')'])
    hold off
    ...
    xlabel('Trial number')
    ylabel('Speed (cm/s)')
    title(['Mean speed (1/3 track) of trials on ' char(data(this_day).date) '(' char(data(this_day).day) ')'])
    hold off
