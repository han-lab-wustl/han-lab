...
diff_ctrl_ep3_ep1 = diff([cell2mat(cellfun(@(x) x', opto_means, 'UniformOutput', false)); ...
    cell2mat(cellfun(@(x) x', opto_comp_means, 'UniformOutput', false))]);
ratio_ctrl_ep3_ep1 = cell2mat(cellfun(@(x) x', opto_means, 'UniformOutput', ...
    false))./cell2mat(cellfun(@(x) x', opto_comp_means, 'UniformOutput', false));
% exclude cells diff < -4 --> is this ok to do??? find which cells these
% are
ex_diff_opto = diff_opto_ep3_ep1(diff_opto_ep3_ep1>-4);
