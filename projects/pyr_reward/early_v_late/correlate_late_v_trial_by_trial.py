...
for animal in df_plt.index.get_level_values('animals').unique():
    sub = df_plt.loc[animal].sort_index()
    ax.plot(sub.index-2, sub['goal_cell_prop'], color='gray', alpha=0.5, linewidth=1.5)
plt.savefig(os.path.join(savedst, 'allreward_early_v_late_cell_prop.svg'), 
        bbox_inches='tight')
...
ax.set_ylabel('')
fig.suptitle('Early')
plt.savefig(os.path.join(savedst, 'allreward_early_v_late_cell_prop-shuffle_per_an.svg'), 
        bbox_inches='tight')
