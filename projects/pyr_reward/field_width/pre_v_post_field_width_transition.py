...
for ct in ['Pre','Post']:
    ...
    near = sub[sub['epoch']=='epoch1_rz1']['width_cm']
    far  = sub[sub['epoch']=='epoch2_rz3']['width_cm']
    t, p = ttest_rel(near, far)
...
for _, row in posthoc.iterrows():
    ...
    ax.text(x, ymax+3, f'{ct} near v far\np={p:.2g}', ha='center', va='bottom',fontsize=12,
            rotation=45)
plt.savefig(os.path.join(os.path.join(savedst, 'near_to_far_field_width.svg')))
...
for ct in ['Pre','Post']:
    sub = anrzdf[anrzdf['cell_type']==ct]
    near = sub[sub['epoch']=='epoch1_rz3']['width_cm']
    far  = sub[sub['epoch']=='epoch2_rz1']['width_cm']
...
for _, row in posthoc.iterrows():
    ...
    ymax = anrzdf[(anrzdf.cell_type==ct)]['width_cm'].max()
    ax.text(x, ymax+1, stars, ha='center', va='bottom',fontsize=42)
    ax.text(x, ymax+5, f'{ct} far v near\np={p:.2g}', ha='center', va='bottom',fontsize=12,
            rotation=45)
plt.savefig(os.path.join(os.path.join(savedst, 'far_to_near_field_width.svg')))
