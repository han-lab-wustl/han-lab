...
def get_place_field_widths(params_pth,animal,day,ii,goal_window_cm=20,bins=90):
    fall = scipy.io.loadmat(params_pth, variable_names=['coms', 'changeRewLoc', 
            'timedFF', 'ybinned', 'VR', 'forwardvel', 'trialnum', 'rewards', 'iscell', 'bordercells',
            'stat', 'licks'])
    VR = fall['VR'][0][0][()]
    ...
    pcs = [xx for xx in pcs if xx not in goal_all]
    # example:
    plt.close('all')
    # rectangular method
    alldf=[]
    ...
    for ep in range(len(tcs_correct)):
        ...
        # w = np.array(w)
        widths_per_ep.append(w)
        peak_per_ep.append([np.quantile(tc,.75) for tc in tcs_correct_abs[ep,pcs]])
    widths_per_ep=np.array(widths_per_ep)
    df = pd.DataFrame()
    df['width_cm'] = np.concatenate(widths_per_ep)
    df['75_quantile'] = np.concatenate(peak_per_ep)
    ...
    try: # eg., suppress after validation
        ii=0
        plt.figure()
        plt.plot(tcs_correct_abs[:,pcs[ii],:].T)
        plt.title(f"{df.loc[df.cellid==pcs[ii], 'width_cm'].values}")
        plt.show()
    except Exception as e:
        print(e)
...
def get_beh_in_field(params_pth,animal,day,ii,goal_window_cm=20,bins=90):
    ...
    tcs_correct, coms_correct, tcs_fail, coms_fail, ybinned_dt = make_tuning_curves_by_trialtype_w_darktime(eps,rewlocs,
        ...
        Fc3,trialnum, rewards,forwardvel,scalingf,bin_size_dt,
        bins=bins_dt)  
    ybinned_dt=np.concatenate(ybinned_dt)
    goal_window = goal_window_cm*(2*np.pi/track_length) # cm converted to rad
    # change to relative value 
    coms_rewrel = np.array([com-np.pi for com in coms_correct])
    perm = list(combinations(range(len(coms_correct)), 2)) 
    ...
    try: # eg., suppress after validation
        ...
        plt.figure()
        plt.plot(tcs_correct[:,goal_all[ii],:].T)
        plt.title(f"{df.loc[df.cellid==goal_all[ii], 'width_cm'].values} \n {df.loc[df.cellid==goal_all[ii], 'vel_in_field_cm_s'].values}")
        plt.show()
    except Exception as e:
        print(e)
    #add
