...
from projects.pyr_reward.field_width.width import get_place_field_widths
# import condition df
...
for ii in range(len(conddf)):
    ...
    if (animal!='e217') & (conddf.optoep.values[ii]<2):
        ...
        print(params_pth)
        df, lick_df = get_place_field_widths(params_pth,animal,day,ii,goal_window_cm=goal_window_cm)
        dfs.append(df)
...
anrzdf=anrzdf[(anrzdf.animal!='e189') & (anrzdf.animal!='e190')& (anrzdf.animal!='e200')]
sns.stripplot(x='epoch',y='width_cm',data=anrzdf,color=color,alpha=0.7,dodge=True,s=s)
h_strip, l_strip = ax.get_legend_handles_labels()
sns.boxplot(x='epoch',y='width_cm',color=color,data=anrzdf,
...
for ct in ['Place']:
    ...
    t, p = scipy.stats.wilcoxon(near, far)
...
for _, row in posthoc.iterrows():
    ...
    ax.text(x, ymax+5, f'{ct} near v far\np={p:.2g}', ha='center', va='bottom',fontsize=12,
            rotation=45)
plt.savefig(os.path.join(os.path.join(savedst, 'near_to_far_place_field_width.svg')))
#%%
...
sns.stripplot(x='epoch',y='width_cm',data=rzdf,color=color,alpha=0.05,dodge=True)
...
anrzdf=anrzdf[(anrzdf.animal!='e189') & (anrzdf.animal!='e190')& (anrzdf.animal!='e200')]
order = ['rz3', 'rz1']
sns.stripplot(x='epoch',y='width_cm',data=anrzdf,color=color,alpha=0.7,dodge=True,s=s,order=order)
h_strip, l_strip = ax.get_legend_handles_labels()
sns.boxplot(x='epoch',y='width_cm',color=color,data=anrzdf,
        fill=False,order=order,
           showfliers=False)
ax.spines[['top','right']].set_visible(False)
...
for ct in ['Place']:
    ...
    t, p = scipy.stats.wilcoxon(near[~np.isnan(near)], far[~np.isnan(far)])
    results.append({'cell_type':ct, 't_stat':t, 'p_uncorrected':p})
...
for _, row in posthoc.iterrows():
    ...
    ymax = anrzdf[(anrzdf.cell_type==ct)]['width_cm'].max()
    ax.text(x, ymax, stars, ha='center', va='bottom',fontsize=42)
    ax.text(x, ymax, f'{ct} far v near\np={p:.2g}', ha='center', va='bottom',fontsize=12,
            rotation=45)
plt.savefig(os.path.join(os.path.join(savedst, 'far_to_near_place_field_width.svg')))
#%%
...
rzdf = rzdf[rzdf.cell_type == 'Place'].drop(columns=['cell_type'])
...
plt.savefig(os.path.join(os.path.join(savedst, 'lick_place_field_width.svg')))
#%%
...
plt.savefig(os.path.join(os.path.join(savedst, 'vel_place_field_width.svg')))
