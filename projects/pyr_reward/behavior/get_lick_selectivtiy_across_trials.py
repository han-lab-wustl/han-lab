...
for i in range(4):
    ...
    ax.axvline(13,color='b',linewidth=3,label='Label 8 trials')
    if i==0: 
...
for i in range(4):
    ...
    for ll,ls in enumerate(lick_tcs_ep_):
        lick_tcs_ep[ll,:len(ls)]=ls
    ax.plot(np.nanmedian(lick_tcs_ep,axis=0),color=colors[i],label=f'Epoch {i+1}, {lick_tcs_ep.shape[0]} sessions')
