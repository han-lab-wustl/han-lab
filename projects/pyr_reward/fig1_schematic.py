...
# mark COMs
for c, col in zip(coms, ['k','gray','teal']):
    axs[0].axvline(c, color=col, ls='-.', alpha=0.7)
...
for i, (rloc, col) in enumerate(zip(reward_locs, colors)):
    ...
    axs[1].axvline(coms_reward[i], color=col, ls='-.', alpha=0.7, label=f'COM, epoch {i+1}')
    ...
    axs[1].text((rloc+coms_reward[i])/2, 0.35+0.2*i,
                f"{dist} cm", ha='center', va='bottom', color=col)
...
fig.suptitle('Pyramidal cell criteria')
axs[1].legend()
