...
from placecell import make_tuning_curves_radians_by_trialtype, intersect_arrays, \
    make_tuning_curves_trial_by_trial
...
#%%    
trialstates, licks, tcs, coms=make_tuning_curves_trial_by_trial(eps,rewlocs,lick,ybinned,rad,Fc3[:,goal_cells],trialnum,
            rewards,forwardvel,rewsize,bin_size,lasttr=8,bins=90)
#%%
# trial state only
fig,axes=plt.subplots(nrows=len(trialstates),ncols=1,sharex=True,figsize=(2,8))
for i in range(len(trialstates)):
    im=axes[i].imshow(np.array([trialstates[i]]).T,cmap='Reds')
    axes[i].set_xticklabels('')

# per epoch trial bu trial with trailstate
fig,axes=plt.subplots(nrows=len(tcs[ep]),ncols=len(tcs),figsize=(8,8),sharex=True)
axes= axes.flatten()
j=0
for ep in range(len(tcs)):
    for i in range(len(tcs[ep])):
        im=axes[j].imshow(tcs[ep][i],vmin=0,vmax=0.5)
        axes[j].set_title(f'Epoch {ep+1}\nPost-reward cell {i}')
        axes[j].set_aspect('equal')
        j+=1
        # fig.colorbar(im, orientation='vertical')
fig.tight_layout()
plt.subplots_adjust(wspace=0, hspace=0) # Set horizontal and vertical spacing to zero
#%%
# vs. far reward

fig,axes=plt.subplots(nrows=len(tcs[0]),ncols=len(tcs),figsize=(8,8),sharex=True)
axes= axes.flatten()
j=0
for ep in range(len(tcs)):
    for i in range(len(tcs[ep])):
...
for ep in range(len(tcs)):
    for i in range(len(tcs[ep])):
        ...
        axes[j].set_title(f'Epoch {ep+1}\nPre-reward cell {i}')
        axes[j].set_aspect('equal')
        axes[j].axvline(45,color='w',linestyle='--')
        j+=1
        # fig.colorbar(im, orientation='vertical')
fig.tight_layout()
plt.subplots_adjust(wspace=0, hspace=0) # Set horizontal and vertical spacing to zero
