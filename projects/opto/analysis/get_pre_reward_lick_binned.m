...
pertile = 0.3; % count licks w/in this percentile
...
prerewlicks = sum(flick((ypos>(rewloc-(rewloc*pertile))) & ypos<rewloc));%/(sum(time_)/60)*30; % /s
...
alllicks = sum(flick(ypos<(rewloc) | ypos>(rewloc+rewloc*pertile)));%;/(sum(time_)/60)*30; % /s
