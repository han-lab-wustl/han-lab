...
for kk, lbl in enumerate(lbls):
   tcs_correct_pc,tcs_correct_gc,coms_correct_pc,coms_correct_gc, rewlocs, eptest=datarasters[kk]
   # place
   ...
   if kk == 0:
      ...
      ax.set_xlabel('Track position (cm)')
      cbar_ax = fig.add_axes([.88, .55, 0.015, 0.3])  # [left, bottom, width, height]
   ...
   if kk == 0:
      ...
      ax.set_xlabel('Track position (cm)')
      cbar_ax = fig.add_axes([.88, .1, 0.015, 0.3])  # [left, bottom, width, height]
      cbar = fig.colorbar(im, cax=cbar_ax)
      cbar.set_label(f'$\Delta$ F/F')
