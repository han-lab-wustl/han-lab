...
for ii in range(len(conddf)):
    ...
    if conddf.in_type.values[ii]=='vip':# and conddf.animals.values[ii]!='e217':
        ...
        threshold=0.99
        differentially_inactivated_cells = find_differentially_inactivated_cells(normalize_2d_array(tc1_late[:, :int(rewlocs[comp[1]]/bin_size)]), normalize_2d_array(tc2_late[:, :int(rewlocs[comp[1]]/bin_size)]), threshold, bin_size)
        differentially_activated_cells = find_differentially_activated_cells(normalize_2d_array(tc1_late[:, :int(rewlocs[comp[1]]/bin_size)]),normalize_2d_array(tc2_late[:, :int(rewlocs[comp[1]]/bin_size)]), threshold, bin_size)
        coms = fall['coms'][0]
...
df['animal'] = conddf.animals.values[(conddf.in_type.values=='vip')]
...
df2['animal'] = conddf.animals.values[(conddf.in_type.values=='vip')]
...
quadrant = 'pre_post'
ax = sns.barplot(x="cond", y=quadrant,hue='opto', data=df,fill=False,
                palette={False: "slategray", True: "red"})
ax = sns.stripplot(x="cond", y=quadrant,hue='opto', data=df, 
                palette={False: "slategray", True: "red"})
...
pre_post = df.loc[(df.index.get_level_values('opto')==True) & (df.index.get_level_values('cond')=='active'), quadrant].values
pre_post_opto = df.loc[(df.index.get_level_values('opto')==False) & (df.index.get_level_values('cond')=='active'), quadrant].values

# pre_post = df.loc[(df.opto==True) & (df.cond=='inactive'), quadrant].values
# pre_post_opto = df.loc[(df.opto==False) & (df.cond=='inactive'), quadrant].values

scipy.stats.ttest_rel(pre_post[~np.isnan(pre_post)], pre_post_opto[~np.isnan(pre_post_opto)])
