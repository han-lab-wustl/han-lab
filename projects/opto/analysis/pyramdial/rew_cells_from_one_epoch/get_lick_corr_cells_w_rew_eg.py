...
for ii in range(len(conddf)):
   ...
   if ii!=202 and optoep<2 and 'vip' not in in_type:
      ...
      if len(com_goal_farrew)>0:
         ...
         goal_cells=[]    
      # num lick corr cells
      rew_not_in_lick_corr = [xx for xx in goal_cells if xx not in lick_corr_cells_ep1]
      lick_corr_cells_not_rew_cells = [xx for xx in lick_corr_cells_ep1 if xx not in goal_cells]
      # fig, axes = plt.subplots(ncols=3)
      # for i in range(3):
      #    ax = axes[i]
      #    inds = np.array(rew_not_in_lick_corr)
      #    data = tcs_correct[i, inds][np.argsort(coms_correct[0, inds])]
      #    ax.imshow(normalize_rows(data), aspect='auto', vmin=0, vmax=1)
      #    ax.axvline(75, color='w', linestyle='--', linewidth=2)
      # axes[0].set_title('Reward cells that are not lick correlated')
      # fig, axes = plt.subplots(ncols=3)
      # for i in range(3):
      #    ax = axes[i]
      #    inds = np.array(lick_corr_cells_not_rew_cells)
      #    data = tcs_correct[i, inds][np.argsort(coms_correct[0, inds])]
      #    ax.imshow(normalize_rows(data), aspect='auto', vmin=0, vmax=1)
      #    ax.axvline(75, color='w', linestyle='--', linewidth=2)
      # axes[0].set_title('Lick cells that are not reward cells')
      # fig, axes = plt.subplots(ncols=3)
      # for i in range(3):
      #    ax = axes[i]
      #    inds = np.array(lick_corr_cells_ep1)
      #    data = tcs_correct[i, inds][np.argsort(coms_correct[0, inds])]
      #    ax.imshow(normalize_rows(data), aspect='auto', vmin=0, vmax=1)
      #    ax.axvline(75, color='w', linestyle='--', linewidth=2)
      # axes[0].set_title('All lick cells')
      # fig, axes = plt.subplots(ncols=3)
      # for i in range(3):
      #    ax = axes[i]
      #    inds = np.array(goal_cells)
      #    data = tcs_correct[i, inds][np.argsort(coms_correct[0, inds])]
      #    ax.imshow(normalize_rows(data), aspect='auto', vmin=0, vmax=1)
      #    ax.axvline(75, color='w', linestyle='--', linewidth=2)
      # axes[0].set_title('All rew cells')
