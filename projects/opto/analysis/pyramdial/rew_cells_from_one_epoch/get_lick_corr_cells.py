...
for ii in range(len(conddf)):
   ...
   if ii!=202:
      ...
      # abs position
      # all trials
      tcs_correct_abs, coms_correct_abs,tcs_fail_abs, coms_fail_abs= make_tuning_curves(eps,rewlocs,ybinned,Fc3,trialnum,rewards,forwardvel,rewsize,bin_size,lasttr=16,bins=150)
      ...
      lick_correct_abs, _,lick_fail_abs,__ = make_tuning_curves(eps,rewlocs,ybinned,np.array([lick_rate]).T,trialnum,rewards,forwardvel,rewsize,bin_size,lasttr=16,bins=150)
      vel_correct_abs, _,vel_fail_abs,__ = make_tuning_curves(eps,rewlocs,ybinned,np.array([forwardvel]).T,trialnum,rewards,forwardvel,rewsize,bin_size,lasttr=16,bins=150)
      # beh rew aligned
      ...
      lick_tc_cs = [[spearmanr(tcs_correct_abs[ep,cll,:], lick_correct_abs[ep][0])[0] for cll in per_ep_cll] for per_ep_cll in ep_nearrew_cells]
      # get high correlated cells
...
df['epoch']=np.concatenate([np.arange(len(v[1]))+1 for k,v in datadct.items()])
df['num_lick_corr'] = np.concatenate([v[2] for k,v in datadct.items()])
df['num_near_rew'] = np.concatenate([v[3] for k,v in datadct.items()])
df['animals']=np.concatenate([[kk.split('_')[0]]*len(v[1]) for kk,v in datadct.items()])

df['days']=np.concatenate([[kk.split('_')[1]]*len(v[1]) for kk,v in datadct.items()]).astype(int)
df=df[df.num_near_rew>0]
df = pd.merge(df, conddf, on=['animals', 'days'], how='left')
df['condition']=[xx if 'vip' in xx else 'ctrl' for xx in df.in_type.values]
df['opto']=df.optoep>1
df=df[df.epoch==2]
sns.barplot(x='condition',y='num_near_rew', hue='opto',data=df)