...
# df['rewzone'] = np.concatenate([v[1] for k,v in datadct.items()])
df['epoch']=np.concatenate([['prev', 'opto'] for k,v in datadct.items()])
df['activity_lick_corr_ep1'] = np.concatenate([np.nanmedian(v[4],axis=1) for k,v in datadct.items()])
df['av_lick_corr'] = np.concatenate([[np.nanmedian(xx) for xx in v[2]] for k,v in datadct.items()])
df['animals']=np.concatenate([[kk.split('_')[0]]*2 for kk,v in datadct.items()])
df['days']=np.concatenate([[kk.split('_')[1]]*2 for kk,v in datadct.items()]).astype(int)
# df=df[df.num_lick_corr>0]
...
# df=df[~((df.animals=='z14')&((df.days<33)))]
df=df[~((df.animals=='z16')&((df.days>13)))]
# df=df[~((df.animals=='z17')&((df.days<9)))]
# df=df[~((df.animals=='z15')&((df.days.isin([7,12,16]))))]
df=df[~((df.animals=='e217')&((df.days<9)|(df.days.isin([21,29,30,26,29]))))]
df=df[~((df.animals=='e216')&((df.days<32)|(df.days.isin([47,55,57]))))]
df=df[~((df.animals=='e200')&((df.days.isin([67,68,81]))))]
...
fig, ax = plt.subplots(figsize=(5,4))
pl = {'opto': 'red', 'prev': 'k'}
sns.barplot(x='condition',y='activity_lick_corr_ep1', hue='epoch',data=df,fill=False,errorbar='se',palette=pl,hue_order=hue_order,legend=False)
sns.stripplot(x='condition',y='activity_lick_corr_ep1', hue='epoch',data=df,dodge=True,palette=pl,s=10,alpha=0.7,hue_order=hue_order)
...
line_offset = 0.02
line_height = 0.003
asterisk_y_offset = 0.002

# For saving the max y value (for proper line placement)
y_max = df['activity_lick_corr_ep1'].max()

for cond in conds:
   subdf = df[df['condition'] == cond]
   pre = subdf[subdf['epoch'] == 'prev']['activity_lick_corr_ep1'].values
   post = subdf[subdf['epoch'] == 'opto']['activity_lick_corr_ep1'].values
   ...
   x = x_locs[cond]
   height = max(np.nanmax(pre), np.nanmax(post)) + line_offset-.02
   h = line_height
   ...
   animals = subdf['animals'].unique()
   for animal in animals:
      a_pre = subdf[(subdf['animals'] == animal) & (subdf['epoch'] == 'prev')]['activity_lick_corr_ep1'].values
      a_post = subdf[(subdf['animals'] == animal) & (subdf['epoch'] == 'opto')]['activity_lick_corr_ep1'].values
      if len(a_pre) > 0 and len(a_post) > 0:
         ax.plot([x - 0.2, x + 0.2], [a_pre[0], a_post[0]], color='gray', alpha=0.5, lw=1.5)
         
ax.set_ylabel('$\Delta F/F$ lick correlated cells')
plt.savefig(os.path.join(savedst, f'supp_fig5_mean_dff_lick_correlation.svg'), bbox_inches='tight')
