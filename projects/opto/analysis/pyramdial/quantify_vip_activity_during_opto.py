...
for m, mouse_name in enumerate(mice):
    ...
    opto_ep = opto_ep_s[m]
    for dyind,dy in enumerate(days):
...
# get rew loc
rl=rewloc[[0,1]]
axes[0].plot(prevepdff[x1:x2,2], 'k')
axes[1].plot(optodff[x1:x2,2], 'r')
ax2_0 = axes[0].twinx()
...
ax2_0.tick_params(axis='y', labelcolor='gray')
add_reward_zone_patches(ax2_0, yposopto, x1, rl[1]-5,color='mediumturquoise')
