...
% 20 cm before reward
lickrewzone = lickbin(ceil(rewloc_bin)-20:ceil(rewloc_bin-rewsize_bin/2));
try % if the opposite area is too close to the end
    lickoppzone = lickbin(ceil(opp_rewloc)-20:ceil(opp_rewloc-rewsize_bin/2));
catch
    lickoppzone = lickbin(ceil(opp_rewloc)-10:ceil(opp_rewloc-rewsize_bin/2)); % take only 10 cm before
end

lick_selectivity = (median(lickrewzone)-median(lickoppzone))/(median(lickrewzone)+median(lickoppzone));
