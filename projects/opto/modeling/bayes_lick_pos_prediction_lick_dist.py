...
class BNN_Conv1D_Rate(nn.Module):
   def __init__(self, input_channels, dropout_rate=0.3):
      super().__init__()
      self.conv1 = nn.Conv1d(in_channels=input_channels, out_channels=32, kernel_size=3, padding=1)
      self.drop1 = nn.Dropout(dropout_rate)
      self.conv2 = nn.Conv1d(32, 32, kernel_size=3, padding=1)
      self.drop2 = nn.Dropout(dropout_rate)
      self.out = nn.Conv1d(32, 1, kernel_size=1)  # output per time point
   ...
   def forward(self, x):
      ...
      x = self.drop2(x)
      return self.out(x).squeeze(1)  # shape: (batch, time)
...
for ii in iis:
   ...
   for ep in range(len(eps)-1):
      ...
      lick_position_rel = lick_rate[eprng]#lick_position[eprng]-(rewlocs[ep])
      ypos = ybinned[eprng]
      lick_position_rel = lick_position_rel.astype(float)
      lick_position_rel[lick_rate[eprng]>8]=0
      lick_position_rel[licks[eprng]==0]=0
      # trial_X = []
      # trial_y = []
   ...
   trial_lickrate = np.zeros((len(trial_y), max_time))

   for trind,trx in enumerate(trial_X):
      trial_fc[trind,:len(trx)]=trx
      trial_lickrate[trind, :len(trial_y[trind])] = trial_y[trind]
   # only corrects
   X = np.stack(trial_fc)   # shape (n_trials, time, n_cells)
   y = np.array(trial_lickrate).squeeze()  # shape (n_trials,)
   from sklearn.model_selection import train_test_split
   import torch
   from torch.utils.data import TensorDataset, DataLoader
   ...
   model = BNN_Conv1D_Rate(input_channels=X_train.shape[1], dropout_rate=0.3)
   ...
   pred_com_licks = y_pred_mean
   real_com_licks = y_test
   ...
   for tr in range(len(test_st_id)):
      plt.figure()
      plt.plot(real_com_licks[test_st_id][tr].T,color='seagreen',label='correct')
      plt.plot(pred_com_licks[test_st_id][tr].T,color='y',label='correct')
      plt.xlabel("Time in trial (Hz)")
      plt.ylabel("Lick rate")
      r2 = r2_score(y_test, y_pred_mean)
      plt.title(f'{animal}, {day}, r2: {r2}')
      plt.legend()
   for tr in range(len(test_flind)):      
      plt.figure()
      plt.plot(real_com_licks[test_flind][tr].T,color='firebrick',label='incorrect')
      plt.plot(pred_com_licks[test_flind][tr].T,color='y',label='incorrect')
      plt.ylabel("Lick rate")
      r2 = r2_score(y_test, y_pred_mean)
      plt.title(f'{animal}, {day}, r2: {r2}')
      plt.legend()
