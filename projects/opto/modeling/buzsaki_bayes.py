   ...
   for ep in range(len(eps)-1):
      ...
      for tr in unique_trials:
         ...
         if fc_trial.shape[0] >= 10 and not np.isnan(avg_lick_pre_rew) and tr>2:#; dont exclude probes for now
            ...
            trial_y.append(rewlocs[ep]+rewsize/2)
            trial_pos.append(ypos_tr)
            ep_trials.append(ep+1)
      from scipy.stats import gaussian_kde

      def compute_tuning_curves(fc3, ybinned, goal_zone, n_pos_bins, n_goals):
         n_trials, T, n_cells = fc3.shape
         tuning_curves = np.zeros((n_cells, n_pos_bins, n_goals))

         for cell in range(n_cells):
            for g in range(n_goals):
                  # Select relevant time bins
                  activity = []
                  pos = []
                  for trial in range(n_trials):
                     if goal_zone[trial] == g:
                        activity.append(fc3[trial, :, cell])
                        pos.append(ybinned[trial, :])
                  activity = np.concatenate(activity)
                  pos = np.concatenate(pos)

                  # Estimate mean activity per position bin using KDE
                  for p in range(n_pos_bins):
                     in_bin = pos == p
                     if in_bin.sum() > 0:
                        tuning_curves[cell, p, g] = activity[in_bin].mean()
         return tuning_curves
      
      def build_transition_matrix(n_pos, n_goals, sigma2=25, dt=0.01):
         P_pos = np.zeros((n_pos, n_pos))
         for i in range(n_pos):
            for j in range(n_pos):
                  P_pos[i, j] = np.exp(-((i - j)**2) / (2 * sigma2))
         P_pos /= P_pos.sum(axis=1, keepdims=True)

         P_goal = np.full((n_goals, n_goals), (1 - 0.9**dt) / (n_goals - 1))
         np.fill_diagonal(P_goal, 0.9**dt)

         return P_pos, P_goal
      
      def run_bayesian_filter(fc3_test, tuning_curves, P_pos, P_goal):
         T, n_cells = fc3_test.shape
         n_pos, n_goals = tuning_curves.shape[1], tuning_curves.shape[2]

         posterior = np.ones((n_pos, n_goals)) / (n_pos * n_goals)
         posteriors = []

         for t in range(T):
            likelihood = np.ones((n_pos, n_goals))
            for cell in range(n_cells):
                  lam = tuning_curves[cell]
                  obs = fc3_test[t, cell]
                  likelihood *= np.exp(-lam) * lam**obs / np.maximum(1, scipy.special.factorial(obs))

            # Transition step
            pred = (P_pos.T @ posterior) @ P_goal.T
      ...
      def detect_goal_segments(goal_post, penalty=5):
         model = "l2"
         algo = rpt.Pelt(model=model).fit(goal_post)
...
n_cells = fc3.shape[2]
n_bins = np.max(ybinned) + 1
n_goals = np.max(goal_zone) + 1

# Initialize tuning curves: cells × position × goal
tuning_curves = np.zeros((n_cells, n_bins, n_goals))

for g in range(n_goals):
   trial_inds = np.where(goal_zone == g)[0]
   for c in range(n_cells):
      counts = np.zeros(n_bins)
      occupancy = np.zeros(n_bins)
      for tr in trial_inds:
         for t in range(fc3.shape[1]):
               pos = ybinned[tr, t]
               if np.isnan(pos):
                  continue
               counts[int(pos)] += fc3[tr, t, c]
               occupancy[int(pos)] += 1
      # Avoid divide-by-zero
      tuning_curves[c, :, g] = gaussian_filter1d(counts / (occupancy + 1e-6), sigma=1)
   ...
   vmin = np.nanmin(np.concatenate([y[test_idx], y_pred_mean]))
   vmax = np.nanmax(np.concatenate([y[test_idx], y_pred_mean]))
   ...
   ax.set_title(f'{animal}, {day}, r2: {r2:.3g}, mae: {mae:.2g} cm')
   ax.legend()
   pdf.savefig(fig)
   plt.show()   
