...
from projects.opto.behavior.behavior import consecutive_stretch
...
for kk,ii in enumerate(iis):
   ...
   trialnum_global = np.zeros_like(trialnum)
   trial_offset = 0  # running counter
   for i in range(len(eps)-1):
      start, end = eps[i], eps[i+1]
      epoch_trials = trialnum[start:end]
      # Get unique trial numbers in order
      unique_trials = np.unique(epoch_trials)
      # Map epoch-local trial numbers to session-global trial numbers
      trial_map = {local: trial_offset + i for i, local in enumerate(unique_trials)}
      trialnum_global[start:end] = np.array([trial_map[t] for t in epoch_trials])
      
      trial_offset += len(unique_trials)  # increment by number of trials in epoch
   # incorrect licks
   trials=trialnum_global[mask]
   incorrlick=np.zeros_like(lick[mask])
   for trial in np.unique(trials):
      tr = trials==trial
      # check if probe
      probe=np.unique(trialnum[mask][trials==trial])
      if not 0 in probe and not 1 in probe and not 2 in probe:
   ...
   ax.set_ylim([0, 350])   
   # plt.tight_layout()
   plt.savefig(os.path.join(savedst, f'supp_fig5_beh_{animal}_{day}.svg'),bbox_inches='tight')