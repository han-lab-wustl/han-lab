...
for dd,day in enumerate(conddf.days.values):
    ...
    rates_opto, rates_prev, rate_opto_13, rate_prev_13, lick_prob_opto, \
    ...
    dct['rate_first13']= [rate_prev_13,rate_opto_13]
...
df['rates_opto'] = [dct['rates'][1] for dct in dcts]
df['rates_prev'] = [dct['rates'][0] for dct in dcts]
df['rates13_opto'] = [dct['rate_first13'][1] for dct in dcts]
df['rates13_prev'] = [dct['rate_first13'][0] for dct in dcts]
df['rates13_diff'] = [np.diff(dct['rate_first13'])[0] for dct in dcts]
df['rates13_diff']=df['rates13_diff']*100
...
df=df[~((df.animals=='z14')&((df.days<33)|(df.days.isin([38]))))]
...
ax=axes[0]
dfex = pd.DataFrame()
dfex['rates'] = np.concatenate([bigdf_plot.rates13_opto, bigdf_plot.rates13_prev])
...
sns.barplot(x="condition", y="rates13_diff", hue='condition', data=bigdf_plot,
            palette=pl, errorbar='se', fill=False, ax=ax)
sns.stripplot(x="condition", y="rates13_diff", hue='condition', data=bigdf_plot,
...
y_start = bigdf_plot['rates13_diff'].max() + 1
gap = 2
...
# ax.set_ylim([-35,10])
plt.tight_layout()
fig.suptitle('First 1/3 of epoch')
plt.savefig(os.path.join(savedst, 'p_correct_trials_opto_all_first13.svg'), bbox_inches='tight')
