...
for jj=1:length(cmut_uni)
    ...
    smut_id=mut_id(find(colormut==cmut_uni(jj)));
     smut_vals=mtm_vals_mut(find(colormut==cmut_uni(jj)))
...
for jj=1:length(cdop_uni)
    sdop_id=dop_id(find(colordop==cdop_uni(jj)));
     sdop_vals=mtm_vals_dop(find(colordop==cdop_uni(jj)))
