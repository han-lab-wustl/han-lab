...
for alldays = 1:length(pr_dir0)%[3:12 14:19]%[3:12 13:19]%[3 5:1]%[5:12 14]%[5:12 14]%1:5%26:30%1:33%31:32%30%27:30%21%[1:21]%[1:2 4:5 12:22]%[8:21]%[2:4 6:11]%[1:2 4:5 12:20]%%[1:21]%%
    ...
    for allplanes=1:4
        ...
        if exist( pr_dir, 'dir')
            ...
            speed_thresh = 5%% 5 for early days and 12; %cm/s cut off for stopped
            ...
            if exist('forwardvelALL','var')
                ...
                if size(forwardvelALL,2) == 1
                    moving_middle=get_moving_time_V3(smoothdata(forwardvelALL,'gaussian',round(gauss_win*1.5)),speed_thresh,Stopped_frame,speedftol);
                else
                    moving_middle=get_moving_time_V3(smoothdata(forwardvelALL,'gaussian',round(gauss_win*1.5))',speed_thresh,Stopped_frame,speedftol);
                end
                ...
                idx_rm=(stop_success_tmpts+post_win_framesALL+1)>length(utimedFF);
                rm_idx=find(idx_rm==1)
                stop_success_tmpts(rm_idx)=[];
                ...
                for stamps=1:length(stop_success_tmpts)-1
                    currentnrrewstopidxperplane = find(timedFF>=utimedFF(stop_success_tmpts(stamps)),1);
                    dop_success_peristop(stamps,:)= norm_base_mean(currentnrrewstopidxperplane-pre_win_frames:currentnrrewstopidxperplane+post_win_frames);
                ...
                if ~isempty(nonrew_stop_success_tmpts)
                       nonrew_stop_success_tmpts(1)=[];
                    for stamps=1:length(nonrew_stop_success_tmpts)-1
                ...
                if ~isempty(rew_stop_success_tmpts)
                      rew_stop_success_tmpts(1)=[];
                    for stamps=1:length(rew_stop_success_tmpts)-1
...
%                 subplot(ceil(sqrt(length(pr_dir0))),ceil(sqrt(length(pr_dir0))),cnt),
                ...
                hold on
                plot(frame_time*(-pre_win_frames)*numplanes:frame_time*numplanes:frame_time*post_win_frames*numplanes,dop_success_perimov);
                hold on
                plot(frame_time*(-pre_win_frames)*numplanes:frame_time*numplanes:frame_time*post_win_frames*numplanes,mean(dop_success_perimov,1),'k','LineWidth',2);
                ...
                savefig(['fullFOV_' 'PERIMOV.fig'])


                find_figure(strcat(mouse_id,'_perilocomotion stopping','_plane',num2str(allplanes)));
%                 subplot(ceil(sqrt(length(pr_dir0))),ceil(sqrt(length(pr_dir0))),cnt),
                ylabel('dF/F')
                ...
                hold on
                plot(frame_time*(-pre_win_frames)*numplanes:frame_time*numplanes:frame_time*post_win_frames*numplanes,dop_success_peristop);
                hold on
                plot(frame_time*(-pre_win_frames)*numplanes:frame_time*numplanes:frame_time*post_win_frames*numplanes,mean(dop_success_peristop,1),'k','LineWidth',2);

                legend(['n = ',num2str(size(dop_success_peristop,1))])
                hold on
                plot(frame_time*(-pre_win_frames)*numplanes:frame_time:frame_time*post_win_frames*numplanes,rescale(mean(roe_success_peristop,1),0.985,0.99),'k--','LineWidth',2)

                text(-5, min(ylim), 'Moving', 'Horiz','left', 'Vert','bottom')
                text(5, min(ylim), 'Stopped', 'Horiz','right', 'Vert','bottom')

                savefig(['fullFOV_' 'PERISTOP.fig'])
            ...
            for roii = 1:size(df_f,1)
                ...
                if exist('single_lick_idx','var')
                    ...
                    if exist('urew_solenoidALL','var')
                        ...
                        singleunrew = 4*unrew_single(find(unrew_single>pre_win_framesALL& unrew_single<length(licksALL)-post_win_framesALL));
                ...
                if exist('forwardvelALL','var')
                    ...
                    if size(forwardvelALL,2) == 1
                        [moving_middle stop]=get_moving_time_V3(forwardvelALL,speed_thresh,Stopped_frame,speedftol);
                    else
                        [moving_middle stop]=get_moving_time_V3(forwardvelALL',speed_thresh,Stopped_frame,speedftol);
                    end
                    mov_success_tmpts = moving_middle(find(diff(moving_middle)>1)+1);
                    ...
                    roidop_2s_peristop=[];  roe_2s_peristop=[];
                    time_stps=2;
                    nframe_stops=time_stps/frame_time;
                    
                    [C_stretch]= istretch(stop,nframe_stops)
                    stops_new=cellfun(@(x) x(1),C_stretch)
                    stop_2s_tmpts=stops_new(find(cellfun(@(x) size(x,2),C_stretch)<nframe_stops))
                    stop_2s_tmpts(1)=[];
                    c=0;
                    for stamps=1:length(stop_2s_tmpts)-1
                        id_stamps=stop_2s_tmpts(stamps);
                        currentidx =  find(timedFF>=utimedFF(id_stamps),1);
                     
                        if currentidx+post_win_frames<length(roinorm_base_mean)&currentidx-pre_win_frames>0
                            c=c+1;
                            roidop_2s_peristop(c,:)= roinorm_base_mean(currentidx-pre_win_frames:currentidx+post_win_frames);
                            roe_2s_peristop(c,:)= forwardvelALL(id_stamps-pre_win_framesALL:id_stamps+post_win_framesALL);
                    ...
                    roidop_4s_peristop=[];  roe_4s_peristop=[];
                    time_stps=4;
                    nframe_stops=time_stps/frame_time;
                    
                    [C_stretch]= istretch(stop,nframe_stops)
                    stops_new=cellfun(@(x) x(1),C_stretch)
                    stop_4s_tmpts=stops_new(find(cellfun(@(x) size(x,2),C_stretch)>nframe_stops))
                    if ~isempty(stop_4s_tmpts)
                        stop_4s_tmpts(1)=[];
                        
                        c=0;
                        for stamps=1:length(stop_4s_tmpts)-1
                            id_stamps=stop_4s_tmpts(stamps);
                            currentidx =  find(timedFF>=utimedFF(id_stamps),1);
                            
                            if currentidx+post_win_frames<length(roinorm_base_mean)&currentidx-pre_win_frames>0
                                c=c+1;
                                roidop_4s_peristop(c,:)= roinorm_base_mean(currentidx-pre_win_frames:currentidx+post_win_frames);
                                roe_4s_peristop(c,:)= forwardvelALL(id_stamps-pre_win_framesALL:id_stamps+post_win_framesALL);
                                
                            end
                        end
                    else
                        
                          roidop_4s_peristop=NaN(length(-pre_win_frames:post_win_frames));
                         roe_4s_peristop=NaN(length(-pre_win_framesALL:post_win_framesALL));
                    ...
                    roidop_2s_perimov=[];  roe_2s_perimov=[];
                    time_stps=2;
                    nframe_stops=time_stps/frame_time;
                    
                    [C_stretch]= istretch(stop,nframe_stops)
                    stops_new=cellfun(@(x) x(end),C_stretch)
                    stop_2s_tmpts=stops_new(find(cellfun(@(x) size(x,2),C_stretch)<nframe_stops))
                    stop_2s_tmpts(1)=[];
                    ...
                    for stamps=1:length(stop_2s_tmpts)-1
                        ...
                        if currentidx+post_win_frames<length(roinorm_base_mean)&currentidx-pre_win_frames>0
                            c=c+1;
                            roidop_2s_perimov(c,:)= roinorm_base_mean(currentidx-pre_win_frames:currentidx+post_win_frames);
                            roe_2s_perimov(c,:)= forwardvelALL(id_stamps-pre_win_framesALL:id_stamps+post_win_framesALL);
                    ...
                    roidop_4s_perimov=[];  roe_4s_perimov=[];
                    time_stps=4;
                    nframe_stops=time_stps/frame_time;
                    ...
                    if ~isempty(stop_4s_tmpts)
                        ...
                        for stamps=1:length(stop_4s_tmpts)-1
                            ...
                            if currentidx+post_win_frames<length(roinorm_base_mean)&currentidx-pre_win_frames>0
                                c=c+1;
                                roidop_4s_perimov(c,:)= roinorm_base_mean(currentidx-pre_win_frames:currentidx+post_win_frames);
                                roe_4s_perimov(c,:)= forwardvelALL(id_stamps-pre_win_framesALL:id_stamps+post_win_framesALL);
                                
                            end
                        end
                    else
                        
                        roidop_4s_perimov=NaN(length(-pre_win_frames:post_win_frames));
                         roe_4s_perimov=NaN(length(-pre_win_framesALL:post_win_framesALL));
                    end
                   ...
                   nonrew_stop_success_tmpts(1)=[];
                    ...
                    if exist('roisingle_traces','var')
                        ...
                        roi_dop_success_peristop{roii} = roidop_success_peristop;
                          ...
                          roi_dop_success_peristop_2s{roii} = roidop_2s_peristop;
                        roi_dop_success_peristop_4s{roii} = roidop_4s_peristop;
                        roi_dop_success_perimov_2s{roii} = roidop_2s_perimov;
                        roi_dop_success_perimov_4s{roii} = roidop_4s_perimov;
                    ...
                    roi_dop_alldays_planes_mov2s{alldays,planeroicount}=roidop_2s_perimov./mean(roidop_2s_perimov(:,1:pre_win_frames),2);
                    if ~isempty(roidop_4s_perimov)
                    roi_dop_alldays_planes_mov4s{alldays,planeroicount}=roidop_4s_perimov./mean(roidop_4s_perimov(:,1:pre_win_frames),2);
                    else
                        roi_dop_alldays_planes_mov4s{alldays,planeroicount} = NaN(1,length(-1*pre_win_frames:post_win_frames));
                        roidop_4s_perimov = NaN(1,length(-1*pre_win_frames:post_win_frames));
                    end
                    roe_alldays_planes_mov2s{alldays,planeroicount}=roe_2s_perimov./mean(roe_2s_perimov(:,1:floor(size(roe_2s_peristop,2)/2)),2);
                     if ~isempty(roe_4s_perimov)
                    roe_alldays_planes_mov4s{alldays,planeroicount}=roe_4s_perimov./abs(mean(roe_4s_perimov(:,1:floor(size(roe_4s_peristop,2)/2)),2));
                     else
                         roe_alldays_planes_mov4s{alldays,planeroicount}= NaN(1,length(-1*pre_win_frames:post_win_frames));
                         roe_4s_perimov= NaN(1,length(-1*pre_win_framesALL:post_win_framesALL));
                     end
                    
                    roi_dop_alldays_planes_stop2s{alldays,planeroicount}=roidop_2s_peristop./mean(roidop_2s_peristop(:,1:pre_win_frames),2);
                    if ~isempty(roidop_4s_peristop)
                    roi_dop_alldays_planes_stop4s{alldays,planeroicount}=roidop_4s_peristop./mean(roidop_4s_peristop(:,1:pre_win_frames),2);
                    else
                        roi_dop_alldays_planes_stop4s{alldays,planeroicount}= NaN(1,length(-1*pre_win_frames:post_win_frames));
                        roidop_4s_peristop= NaN(1,length(-1*pre_win_frames:post_win_frames));
                    end
                    
                    roe_alldays_planes_stop2s{alldays,planeroicount}=roe_2s_peristop./mean(roe_2s_peristop(:,1:floor(size(roe_2s_peristop,2)/2)),2);
                     if ~isempty(roe_4s_peristop)
                    roe_alldays_planes_stop4s{alldays,planeroicount}=roe_4s_peristop./mean(roe_4s_peristop(:,1:floor(size(roe_2s_peristop,2)/2)),2);
                     else
                         roe_alldays_planes_stop4s{alldays,planeroicount}=NaN(1,length(-1*pre_win_frames:post_win_frames));
                         roe_4s_peristop=NaN(1,length(-1*pre_win_framesALL:post_win_framesALL));
                     end
                    ...
                    roi_dop_allsuc_2s_mov(alldays,planeroicount,:)=mean(roidop_2s_perimov./mean(roidop_2s_perimov(:,1:pre_win_frames),2),1);
                    roi_dop_allsuc_2s_stop(alldays,planeroicount,:)=mean(roidop_2s_peristop./mean(roidop_2s_peristop(:,1:pre_win_frames),2),1);
                    
                    
                    roi_dop_allsuc_4s_mov(alldays,planeroicount,:)=mean(roidop_4s_perimov./mean(roidop_4s_perimov(:,1:pre_win_frames),2),1);
                    roi_dop_allsuc_4s_stop(alldays,planeroicount,:)=mean(roidop_4s_peristop./mean(roidop_4s_peristop(:,1:pre_win_frames),2),1);
                    
                    
                    roe_allsuc_mov_2s(alldays,planeroicount,:)=mean(roe_2s_perimov,1);
                    roe_allsuc_stop_2s(alldays,planeroicount,:)=mean(roe_2s_peristop,1);
                    
                    roe_allsuc_mov_4s(alldays,planeroicount,:)=mean(roe_4s_perimov,1);
                    roe_allsuc_stop_4s(alldays,planeroicount,:)=mean(roe_4s_peristop,1);
...
%                     subplot(ceil(sqrt(length(pr_dir0))),ceil(sqrt(length(pr_dir0))),cnt),
                    ...
                    hold on
                    plot(frame_time*(-pre_win_frames)*numplanes:frame_time*numplanes:frame_time*post_win_frames*numplanes,roidop_success_perimov);
                    hold on
                    plot(frame_time*(-pre_win_frames)*numplanes:frame_time*numplanes:frame_time*post_win_frames*numplanes,mean(roidop_success_perimov,1),'k','LineWidth',2);

                    legend(['n = ',num2str(size(roidop_success_perimov,1))])
                    hold on
                    plot(frame_time*(-pre_win_frames)*numplanes:frame_time:frame_time*post_win_frames*numplanes,rescale(mean(roe_success_perimov,1),0.985,0.99),'k--','LineWidth',2)
                    ...
                    savefig(['ROI_' num2str(roii) '_PERIMOV.fig'])
...
%                     subplot(ceil(sqrt(length(pr_dir0))),ceil(sqrt(length(pr_dir0))),cnt),
                    ...
                    hold on
                    plot(frame_time*(-pre_win_frames)*numplanes:frame_time*numplanes:frame_time*post_win_frames*numplanes,roidop_success_peristop);
                    hold on
                    plot(frame_time*(-pre_win_frames)*numplanes:frame_time*numplanes:frame_time*post_win_frames*numplanes,mean(roidop_success_peristop,1),'k','LineWidth',2);

                    legend(['n = ',num2str(size(roidop_success_peristop,1))])
                    hold on
                    plot(frame_time*(-pre_win_frames)*numplanes:frame_time:frame_time*post_win_frames*numplanes,rescale(mean(roe_success_peristop,1),0.985,0.99),'k--','LineWidth',2)
                    ...
                    savefig(['ROI_' num2str(roii) '_PERISTOP.fig'])



                    %%%%% perireward ALLDAYS
                    if exist('roisigle_traces','var')
            ...
            if exist('roi_single_traces','var')
