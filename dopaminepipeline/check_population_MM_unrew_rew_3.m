...
% % % 
% % % 
% % % Rewarded CS and unrewarded CS activity
% % %  	Overlay of CS rewarded and unrewarded CS with speed
% % % Difference in activity and speed behavior in rewarded and unrewarded CS conditions
% % % Significance and population summary of comparison between CSrewarded and unrewarded CS
% % % 
% % % You can also look for other categories if you want. Right now its CSrew vs CS unrewarded
...
%     path='G:\dark_reward\solenoid_unrew_3';
    path = 'F:\reward_omission';
%      path='G:\dark_reward\solenoid_US_reward_only';
...
workspaces={'167_dark_reward_workspace_22.mat','168_dark_reward_workspace_20.mat', '169_dark_reward_workspace_07','171_dark_reward_workspace_21.mat','170_dark_reward_workspace_03.mat' }
...
fileslist={[1:5],[1:5],[1:5],[1:5],[1:4]};
    ...
    cats5={'roi_dop_allsuc_perireward' 'roi_dop_allsuc_perireward_double' 'roi_dop_allsuc_stop'...
        'roi_dop_allsuc_stop_reward' 'roi_dop_allsuc_stop_no_reward' 'roi_dop_allsuc_mov' 'roi_dop_allsuc_unreward_single' 'roi_dop_allsuc_perirewardCS' ,'roi_dop_allsuc_perireward_usonly'...
        }
    cats6={'roe_allsuc_perireward' 'roe_allsuc_perireward_double'  'roe_allsuc_stop'...
        'roe_allsuc_stop_reward' 'roe_allsuc_stop_no_reward' 'roe_allsuc_mov' 'roi_roe_allsuc_perireward_unrewardCS' 'roi_roe_allsuc_perirewardCS','roi_roe_allsuc_rewusonly_lick' }
    cats7={'roi_dop_alldays_planes_perireward','roi_dop_alldays_planes_perireward_double','roi_dop_alldays_planes_success_stop'...
        'roi_dop_alldays_planes_success_stop_reward','roi_dop_alldays_planes_success_stop_no_reward','roi_dop_alldays_planes_success_mov', 'roi_dop_alldays_planes_unreward_single','roi_dop_alldays_planes_periCS'...
        'roi_dop_alldays_planes_periUS'};
        ...
        if strcmp(roevariablename(end-1:end),'_0')
            ...
            if allcat>7
             dopvariable_alldays=eval(dopvariablename_alldays);
                ...
                dopvariable_alldays=eval(dopvariablename_alldays);
                dopvariable_alldays = cellfun(@transpose,dopvariable_alldays,'UniformOutput',0);
            end
        end
        %     color = Pcolor;
        ...
        for jj = 1:size(dopvariable,2)
          ...
          subplot(2,length(workspaces),(cnt_2-1)*length(workspaces)+currmouse)
            ...
            df_f2all{currmouse,cnt_2}=dopvariable;
             df_f2_CS{currmouse,2}=roi_dop_alldays_planes_periCS;
             df_f2_CS{currmouse,1}=roi_dop_alldays_planes_unreward_single;
             if jj == 1
                minyax = min(yax);
                maxyax = max(yax);
...
%             end
            ...
            if cnt_2==1
                title(strcat(workspaces{currmouse}(1:3),'Unrewarded CS'))
            else
                  title(strcat(workspaces{currmouse}(1:3),'rewarded CS'))
            end
        ...
        ac_pstmouse{currmouse,jj} = pst_ac;
            ...
            allmouse_dop{allcat-6}{currmouse,jj}=squeeze(dopvariable(:,jj,:));
            allmouse_roe{allcat-6}{currmouse,jj}=squeeze(roevariable(:,1,:));
            allmouse_time{allcat-6}{currmouse,jj} = xax;
             allmouse_dop_alldays{allcat-6}{currmouse,jj}=cell2mat(cat(1,dopvariable_alldays(:,jj)'))';
        end
...
%         find_figure(strcat('significance allmouse',cats5{allcat}))
    ...
    scatter(ones(size(mtm_vals_SOdop)),mtm_vals_SOdop,25,'r','filled','Jitter','on','Jitteramount',0.03)
    hold on
    scatter(ones(size(mtm_vals_SOmut))*1.5,mtm_vals_SOmut,25,[0.5 0 0],'filled','Jitter','on','Jitteramount',0.03)
     scatter(ones(size(mtm_vals_dop))*2.5,mtm_vals_dop,25,[0 0 1],'filled','Jitter','on','Jitteramount',0.03)
    scatter(ones(size(mtm_vals_mut))*3,mtm_vals_mut,25,[0 0 0.5],'filled','Jitter','on','Jitteramount',0.03)
    xlim([0.5 3.5])
    [~,pl] = ttest(mtm_vals_SOdop-1);
    text(1,nanmean(mtm_vals_SOdop),['p:' num2str(pl)])
    
    [~,pl] = ttest(mtm_vals_SOmut-1);
    text(1.5,nanmean(mtm_vals_SOdop),['p:' num2str(pl)])
    
    [~,pl] = ttest(mtm_vals_dop-1);
    text(2.5,nanmean(mtm_vals_SOdop),['p:' num2str(pl)])
    
    [~,pl] = ttest(mtm_vals_mut-1);
    text(3,nanmean(mtm_vals_SOdop),['p:' num2str(pl)])
    ...
    xticks([1 1.5 2.5 3])
    xticklabels({'GrabDA','GRabDA-mut','GrabDA','GrabDA-mut'})
...
xaxn=frame_time*(-pre_win_frames)*numplanes:frame_time:frame_time*post_win_frames*numplanes;                
                ...
                for ll=1:length(win_sig)-mov_win_frames
                    ...
                    var1=mean(squeeze(df_f2all{jj,1}(:,kk,win_find)),2);
                    var2=mean(squeeze(df_f2all{jj,2}(:,kk,win_find)),2);
                    
                
                    vals_1=cellfun(@(x) mean(x(:,win_find),2) ,df_f2_CS{jj,1} ,'UniformOutput' ,false);
                    var1=cat(1,vals_1{:,kk});
                     vals_2=cellfun(@(x) mean(x(win_find,:),1) ,df_f2_CS{jj,2} ,'UniformOutput' ,false);
...
%                 all_sig(jj,kk,:)=sig_all;
                ...
                xlabel('time from CS')
                if kk==4
                    legend({'Rew-Unrew df/f','Rew-Unrew Speed'}, 'Location','southwest')
                end
            end
        ...
        if kk<4
            ...
            ylim(ylims)
            pls.Color(4) = 0.5;
%             legend('Unrewarded CS','Rewarded CS')
        else
...
for jj=1:size(df_f2,1) %% currmouse
    ...
    for kk=1:size(df_f2,2) %%% roi
        for ll=1:2 %%% 1- unrewarded 2-rewarded
            win_sc=[40 42; 41 43; 56 72];
           
            for mm=1:3 %%% 1-40-48 frames (1s) 2-56 -72 frames(2s)
               vals(jj,kk,ll,mm)=mean(mean(squeeze(df_f2all{jj,ll}(:,kk,win_sc(mm,1):win_sc(mm,2))),2));
               win_find=win_sc(mm,1):win_sc(mm,2);
               if ll==1
                 vals_1=cellfun(@(x) mean(x(:,win_find),2) ,df_f2_CS{jj,1} ,'UniformOutput' ,false);
                    var_f{jj,kk}{ll,mm}=cat(1,vals_1{:,kk});
               else
                   vals_1=cellfun(@(x) mean(x(win_find,:),1) ,df_f2_CS{jj,2} ,'UniformOutput' ,false);
...
idz1=[];
    for kk=1:3
        ...
        for ll=1:4
            ...
            mean_val=[mean_val mean(squeeze(vals(1:m_anz,ll,1,kk))) mean(squeeze(vals(1:m_anz,ll,2,kk))) NaN];
        ...
        if kk==1
            title('0.2s after CS')
        elseif kk==2
            title('between 0.5s-1.5s after CS')
        else
            title('between 2s-4s after CS')
        end
...
idz1=[];
    ...
    for kk=2
        ...
        for ll=1:4
            ...
            idz1=[idz1 idz idz+1 NaN];
            mean_val=[mean_val mean(vals_r) mean(vals_ur) NaN];
        ...
        if kk==1
            title('0.2s after CS')
        elseif kk==2
            title('between 0s-0.4s after CS')
        else
            title('between 2s-4s after CS')
        end
    ...
    for mus = 1:size(df_f2_CS,1)-1
        y1 = [y1 nanmean(nanmean(cellfun(@(x) nanmean(nanmean(x(:,41:43),2)),df_f2_CS{mus,1}(:,4),'UniformOutput',1)))];
    ...
    for mus = 1:size(df_f2_CS,1)-1
        y1 = [y1 nanmean(nanmean(cellfun(@(x) nanmean(nanmean(x(:,41:43),2)),df_f2_CS{mus,1}(:,1:3),'UniformOutput',1)))];
        y2 = [y2 nanmean(nanmean(cellfun(@(x) nanmean(nanmean(x(41:43,:),1)),df_f2_CS{mus,2}(:,1:3),'UniformOutput',1)))];

    end
    
    comparestatsplotspair(y2,y1,[0.5 0.5 0.5],[0.1 0.1 0.1],'k',[4 5])
    
    xlim([0.5 5.5])
    xticks([1.5 4.5])
    xticklabels({'SO','All Except SO'})
    chils = get(gca,'Children');
    legend(chils([22 21 11 10]),{'Rewarded CS','Unrewarded CS','Rewarded CS','Unrewarded CS'})
    xlims = xlim;
    plot(xlims,[1 1],'k--')