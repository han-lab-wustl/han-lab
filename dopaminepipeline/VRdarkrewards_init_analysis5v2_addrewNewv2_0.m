...
%         pr_dir=strcat(pr_dir1,'\plane',num2str(plane-1),'\reg_tif\','')
        ...
        if exist( pr_dir, 'dir')
            ...
            if abs(length(base_mean)-length(lickVoltage))>10 %what the heck? but doesn't matter because we don't use the full FOV
            base_mean = zeros(size(lickVoltage));
            end
            ...
            if oldbatch == 1 %old batch remove
                clearvars rew_us_solenoidALL rew_us_solenoidALL urew_solenoid rew_us_solenoid
                dumbfix = find(rewardsALL==2);
                rewardsALL(dumbfix-15) = 1;
                rewardsALL(dumbfix) = 1;
            end
            ...
            if ~isempty(single_rew)
                if single_rew(1) - pre_win_framesALL - 4 <0%remove events too early
                    single_rew(1)=[];
                end
                if single_rew(end) + post_win_framesALL+4 > length(utimedFF)%remove events too late
                    single_rew(end)=[];
                end
            end
...
%                    
                    ...
                    idx_rm= (lick_norew_stop_success_tmpts - pre_win_framesALL) <=0;
                    rm_idx=find(idx_rm==1);
                    lick_norew_stop_success_tmpts(rm_idx)=[];
...
%                     
                    ...
                    if ~isempty(roidop_nolick_peristop_no_reward)
                     ...
                     if ~isempty( roidop_lick_peristop_no_reward)
                        ...
                        roi_dop_allsuc_lick_stop_no_reward(alldays,planeroicount,:) = NaN(1,size(roi_dop_allsuc_lick_stop_no_reward,3));
                    end
                    if ~isempty(roidop_nolick_peristop_no_reward)
                        ...
                        roi_dop_allsuc_nolick_stop_no_reward(alldays,planeroicount,:) = NaN(1,pre_win_frames*2+1);
                    end
