function [binnedPerireward,allbins,rewdFF] = perirewardbinnedvelocity(dFF,rewards,timedFF,range,binsize)
Rewindx = find(rewards);
% binsize = 0.1; %half a second bins
% range = 6; %seconds back and forward in time
rewdFF = zeros(ceil(range*2/binsize),length(Rewindx));
    ...
    for bin = 1:ceil(range*2/binsize)
        ...
        if ~isempty(checks{1})
            ...
            if sum(currentidxlogical)>0
                ...
                rewdFF(bin,rr) = mean(dFF(checkidx),'omitnan');
            else
                rewdFF(bin,rr) = NaN;
            end
        else
...
meanrewdFF = mean(rewdFF,2,'omitnan');
