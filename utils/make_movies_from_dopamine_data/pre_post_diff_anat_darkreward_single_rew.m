...
for alldays=1:length(pr_dir0)%31%41%12%30%ays_check%[1:2 4:5 12:27 29]%[2:3 5:8 10 12:17 21  24 26]%22:29%[1:2 4:5 12:22]%1:21%[1:2 4:5 12:22]%21%[1:2 4:5 12:22]%[1:21]%%[1:2 4:5 12:20]%%[1:21]%
    ...
    for allplanes=1:4
        ...
        for jj=1:length(ntif)
            ...
            singlerew = single_rew(find(single_rew>pre_win_frames&single_rew<length(licks)-post_win_frames-1))-CSUSframelag_win_frames;
            ...
            % [41-35 169-110 594 403]
            x1=41-35; % ZD changed this dim to remove stripy lines
            x2=594;
            y1=169-110;
            y2=403;
            ...
            h0 = find_figure('reward_alig_movie'); h0.Units = 'normalized'; h0.Position = [0 0 1 1]; h1=subplot(1,2,1),imagesc((imresize(squeeze(mean(chone_crop,3)),1/4))); colormap(h1,gray); axis image
            % h0 = find_figure('reward_alig_movie'); h0.Units = 'normalized'; h0.Position = [0 0 1 1]; h1=subplot(1,2,1),imagesc((squeeze(mean(chone_crop,3)))); colormap(h1,gray); axis image
            ...
            cd(pr_dir0{1})
            writerObj = VideoWriter(strcat('plane_',num2str(allplanes),'_PeriRewardDiff3.avi', 'Uncompressed AVI'));
