...
for day_number = 1:length(filename)
    ...
    if mod(day_number,24) == 0
         subplot(min(ceil(length(filename)/4),6),4,24)
    else
...
%         end
        ...
        for mm = 1:length(changeRewLoc)-1 %the rectangle indicating the reward location, overlaps the probe trials referring to the previous reward location
            rectangle('position',[time_min(changeRewLoc(mm)) RewLoc(mm)-rewSize time_min(changeRewLoc(mm+1))-time_min(changeRewLoc(mm)) 2*rewSize], ...
