function code = Track_270cm_10cmRL_ZD_sol2_fewrewmostlystim_optopostrew
...
p = 1; % probability of having probe trial at the beginning
vr.lickThreshold = -0.075; % might need to adjust
vr.goal = 90; % HERE state the automatic reward location
vr.lickControl = 1; % HERE state the if lick is necessary to get reward
vr.posControl = 1;
vr.countControl = 1;
vr.rewardZone =20*vr.gainFactor; % length of reward zone
...
vr.minsuccessCatch = 5;
vr.maxsuccessCatch = 8;
% vr.curCatchInterval = 0;
vr.curCatchInterval = round(rand()*(vr.maxsuccessCatch-vr.minsuccessCatch))+vr.minsuccessCatch; % initialize?
vr.pastCatch = 0;
...
vr.minSuccessfulTrial = 35;
vr.maxSuccessfulTrial = 41;
vr.optoTrigger = {};
% vr.optoEp = [2];
vr.optoEp = [double(1)];
vr.optoIson = 0;
...
% ZD variables
% vr.zones = [101 78 143 111 80]; % sets a specific order of rew zones
vr.zones = [150 150 150]; % single rew zone
...
vr.currentWorld = 1; % default world at start - we only have 2 worlds in this esperiment; ZD CHANGED DEFAULT TO WORLD 2
...
vr.eligible = 0; %this trial is eligible for reward
vr.curRewEpoch =  0;
...
if vr.numProbes>0 && rand < p
    I = (-2:0) ; %GM + ZD addittions to have 3-5 probes
    vr.trialNum(1) = I(randi(vr.numProbes)) ; % ZD added larger range of initialprobes
else
...
if vr.optoIson == 0 % ZD: only post rew opto
    % if they pass the beginning of the reward zone, stim, regardless of
    % trial type
    if vr.trialNum(1,vr.t)>vr.numProbes-1 && ((vr.position(2)>(vr.goal-(vr.rewardZone/2)))) ...
            && sum(vr.optoEp == sum(vr.changeRewLoc~=0))>0  && ~rem(vr.trialNum(1,vr.t),10)==0
        vr.optoIson = 1; % prevents repeated opto
        write(vr.optodeviceObj,[1 2000],'uint16')
        vr.optoTrigger = [vr.optoTrigger {[vr.t,1,2000]}];
    end
end
% opto either after reward or 10 cm around rewloc
...
if double(vr.keyPressed == 52) %ascii code for "4"
    vr.lickThreshold = vr.lickThreshold+0.005;
...
if  vr.isChange==1 && vr.position(2)==vr.beginZone %EPOCH CHANGE
    ...
    % ZD overwrite
    vr.pastCatch = 0 ;
    disp(sum(vr.changeRewLoc>0)+1);
    vr.goal = vr.zones(sum(vr.changeRewLoc>0)+1);
    vr.isChange=0;
    disp('===')
    ...
    vr.changeRewLoc(1,vr.t)=vr.goal;
end
...
if  vr.time2EndCheck == 1
    if  (vr.time(1,vr.t)-vr.time2End)>=vr.endTime % chech the animal stayed at the end for the desired time
        ...
        vr.dp(1:4) = [0 0 0 0];
        vr.sametrial = 0;
        vr.eligible=0; % not eligible for rewards
        vr.trialNum(1,vr.t)=vr.trialNum(1,vr.t)+1;
        % unless trialnum < 8
        if vr.trialNum(1,vr.t)<8
            vr.eligible=1;
        end
        vr.optoIson = 0;
        % vr.curRewEpoch = number of correct trials
        fails = sum(unique(vr.trialNum(1,1:vr.t))>2)-vr.curRewEpoch; % calc num fails
        if (fails-vr.pastCatch == vr.curCatchInterval)
            vr.eligible = 1; % rew these trials but not others
            vr.curCatchInterval = round(rand()*(vr.maxsuccessCatch-vr.minsuccessCatch))+vr.minsuccessCatch;
            vr.catchTrial(1,vr.t) = 1;
            vr.pastCatch = fails;
            if vr.optoIson == 0 % ZD: only post rew opto
                % if they pass the beginning of the reward zone OR ge a reward, stim, regardless of
                % trial type
                if ((vr.position(2)>(vr.goal-(vr.rewardZone/2))) || vr.eligible==0) ...
                        && sum(vr.optoEp == sum(vr.changeRewLoc~=0))>0
                    vr.optoIson = 1; % prevents repeated opto
                    write(vr.optodeviceObj,[1 2000],'uint16')
                    vr.optoTrigger = [vr.optoTrigger {[vr.t,1,2000]}];
                end
...
%         && (sum(double(vr.worlds{vr.currentWorld}.surface.visible(:))) > 0) &&...
%         ~(vr.position(2)>(vr.goal+(vr.rewardZone/2)) || vr.eligible == 0) && rem(vr.trialNum(1,vr.t),2)==1
...
if vr.worlds{vr.currentWorld}.surface.visible(:) == false % prevent the VR from moving when the screen is black
    ...
    if vr.currentWorld == 2 && vr.worldCurrent(check) ~= 1
        ...
        vr.worlds{1, 2}.backgroundColor = vr.world2darkTimeColor; % ZD changed
    end
end
...
VR.wOff = vr.wOff ;
VR.catchTrial=vr.catchTrial; % ZD added to save catch binary
...
settings.minsuccessCatch=vr.minsuccessCatch;
settings.minsuccessCatch=vr.maxsuccessCatch;
...
if exist('name_date_vr','var')
    ...
    save('C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\summary','name_date','-v7.3')
end
