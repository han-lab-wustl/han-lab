function code = Track_180cm_teleport_ROE_HRZfinal_brighter_5cmRewLoc_ZD_sol2_OPTOpostRew
...
vr.zones = [123 123 123 123]; % single rew zone
...
%GM 11/13/2023 Start Opto at start of dark time for sepcific epoch
        % ZD no dark time
        if vr.optoIson == 0
        if sum(vr.optoEp == sum(vr.changeRewLoc~=0))>0 && (vr.trialNum(1,vr.t)>vr.numProbes-1) && (sum(double(vr.worlds{vr.currentWorld}.surface.visible(:))) > 0) && ~(vr.position(2)>(vr.goal+(vr.rewardZone/2)) || vr.eligible == 0) && rem(vr.trialNum(1,vr.t),2)==1
