function code = Track_270cm_to_7m_10cmRewLoc_ZD_remap
...
vr.initialgainFactor = vr.gainFactor; % IF GAIN OF WORLD2 IS DIFFERENT
...
if vr.currentWorld == 2
    % don't do this for zd's version because world is grey
    % vr.worlds{1, 2}.backgroundColor = vr.world2color; % change the VR background color
    vr.gainFactor = 1; % zd switched gain off for 7m world 2
    % multiply rew locs by factor different by world
    %     vr.goal = vr.goal*(7/1.8);
end
...
if  vr.isChange==1 && vr.position(2)==vr.beginZone
    ...
    worldf=(7/1.8); %todo: make modular
    ...
    if vr.automaticExperimentWorld == 1
        currentNRewLoc=sum(vr.changeRewLoc>0);
        % for long world 2
        if (vr.currentWorld == 1 && currentNRewLoc>vr.automaticExperimentWorld1NRewZones*vr.nW1Changes + vr.automaticExperimentWorld2NRewZones*vr.nW2Changes-vr.manualAutoCorrection && vr.currentWorld == 1) || vr.currentWorld == 2
            a=[{103:267} {291:476} {496:641}];%defines the regions, here we have three
            indx=vr.i(sum(vr.changeRewLoc>=1)+1); %choose the region
            tempdist = abs(vr.changeRewLoc(find(vr.changeRewLoc,1,'last'))-a{1,indx}); % which locations are 30 cm apart from the previous one
            rewZoneIdx = find (tempdist > 50); % find only the good indexes
            temp = a{1,indx}(rewZoneIdx(randi(length(rewZoneIdx)))); %randomly choose one of them
            vr.goal = temp; %set the new reward zone
            vr.changeRewLoc(1,vr.t)=temp;
        end
...
VR.scalingFACTOR=vr.initialgainFactor ;
VR.wOff = vr.wOff ;
