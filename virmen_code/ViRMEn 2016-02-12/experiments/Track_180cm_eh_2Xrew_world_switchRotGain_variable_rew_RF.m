function code = Track_180cm_eh_2Xrew_world_switchRotGain_variable_rew_RF
%random foraging
%added or changed code:

%7 lines into the init code:
%vr.binsize = 2.25; %cm, how big each bin is. 
%for arbx = 1:floor(180/vr.binsize)-1 %assigns an index for each bin. exclude 0 to binsize and end to avoid impossible reward
%    vr.binsIndx(arbx) = arbx*vr.binsize;
%end

%added the line: if vr.currentWorld == 1, to distinguish in rules between
%world 1 and world 2. used elseif in order to have 3 worlds
%copied all of world 1 code into world 2

%changed keypress + code to account for changing the reward:
 %if vr.currentWorld == 1; 
%            vr.currentWorld = 2; % set the current world
%  ->         vr.currentGoal = vr.binsIndx(unidrnd(length(vr.binsIndx))); %sets the current goal to random place for foraging
%        else
%            vr.currentWorld = 1;
%  ->          vr.currentGoal = 0;
%      end

%changed how reward position was located for world 2:
%if (vr.position(2)>vr.currentGoal && vr.position(2)<(vr.currentGoal+vr.binsize))
...
if (vr.position(2)>vr.currentGoal && vr.position(2)<(vr.currentGoal+vr.binsize)&& vr.rewardTimer > 1.3)
    identcurrentGoal = vr.currentGoal;
    vr.rewardTimer = 0;
    while (identcurrentGoal == vr.currentGoal) %prevents reward location from being the same
    vr.currentGoal = vr.binsIndx(unidrnd(length(vr.binsIndx)));
