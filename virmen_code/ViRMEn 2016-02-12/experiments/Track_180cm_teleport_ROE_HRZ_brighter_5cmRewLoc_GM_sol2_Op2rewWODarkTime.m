function code = Track_180cm_teleport_ROE_HRZ_brighter_5cmRewLoc_GM_sol2_Op2rewWODarkTime
...
%GM 11/13/2023 Start Opto at start of dark time for sepcific epoch
        % ZD no dark time
        if vr.optoIson == 0
        if sum(vr.optoEp == sum(vr.changeRewLoc~=0))>0 && (vr.trialNum(1,vr.t)>vr.numProbes-1) && (sum(double(vr.worlds{vr.currentWorld}.surface.visible(:))) > 0) && ~(vr.position(2)>(vr.goal+(vr.rewardZone/2)) || vr.eligible == 0)
