function code = Track_180cm_teleport_ROE_conLick_HRZ_EBH0924ImagingTestNOGLOBAL
...
vr.scalingFACTOR = 1; % Gain in the track. The smaller, the longer the track. 1= usual gain. This value is multiplied by num rpm from ROE
p = 0.1 ;%probability of having probe trial at the beginning
...
vr.rewardZone = 15*vr.scalingFACTOR; % length of reward zone 
...
if vr.scalingFACTOR ~= 1
    ...
    if answer_automaticG == 0
        ...
        vr.scalingFACTOR = str2double(answer_automaticGN);
    end
end
...
vr.beginZone = 2; % value 1 does not allow movements
...
vr.t=vr.t+1;
vr.iSync(vr.t)=vr.imageSync;
vr.lickVoltage(vr.t)=vr.currentLick;  %state from where we are getting the lick
%%signal remember to update signal file
vr.time(vr.t)=toc;
vr.roe(vr.t)=vr.count;% Read current count from the quadrature rotary encoder.
vr.timeROE(vr.t)=vr.timeROE; % Time elasped in seconds since Arduino server starts running (double)
vr.lick(vr.t)= vr.currentLick<vr.lickThreshold; %lislick;
% if lickSensor<vr.lickThreshold
%  sound(1) 
% end
...
if vr.trialNum(vr.t)>vr.numProbes-1 && vr.eligible==1 && vr.position(2)>(vr.goal-(vr.rewardZone/2)) && vr.position(2)<(vr.goal+(vr.rewardZone/2))
    if vr.lickControl==0
            vr.isReward=1;
            vr.eligible=0;
    elseif vr.lickControl==1 && vr.lick(vr.t) == 1 
            vr.isReward=1;
            vr.eligible=0;
    end
end
...
if double(vr.keyPressed == 55) && vr.rewardZone>=15 %ascii code for "7"
 ...
 vr.rewardZone =  vr.rewardZone - 5*vr.scalingFACTOR ;
 disp(num2str(vr.rewardZone))
end
...
VR.imageSync=vr.iSync;%2000920. EBH
VR.scalingFACTOR=vr.scalingFACTOR;
VR.wOff = vr.wOff;
