function code = Track_180cm_teleport_ROE_conLick_classicTraining0830
...
vr.lickThreshold = -0.031;% might need to adjust, unfortunately the clampex recording is not reliable, use licksensor_toy
% 
...
vr.random_initiation = 0; % randomly initiate reward location
%
%--------------------------------------------------------------------------
...
if vr.position(2)== vr.beginZone && vr.time(vr.t)>=vr.time2dark+(vr.rewarddarkTime+round(rand*2,1)) % variable length in dark
...
if double(vr.keyPressed == 55) && vr.rewardZone>=15 %ascii code for "7"
 vr.rewardZone =  vr.rewardZone - 5;
    disp('===')
    disp(['Rew zone length = ' num2str(vr.rewardZone)])
end
...
if  vr.isChange==1 && vr.position(2)==vr.beginZone
    if sum(vr.changeRewLoc>=1)<=vr.numMainRewZone-1
        ...
        tempdist = abs(vr.changeRewLoc(find(vr.changeRewLoc,1,'last'))-a{1,indx}); % which locations are closer than 30 cm from the previous one
        ...
        a = (60:1:160);
        tempdist = abs(vr.changeRewLoc(find(vr.changeRewLoc,1,'last'))-a);
        rewZoneIdx = find (tempdist > 30);
        temp = a(rewZoneIdx(randi(length(rewZoneIdx))));
    end
...
if exist('name_date_vr','var')
    save(['C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\VR_data\classicTraining\' name_date_vr '.mat'],'VR')
end
