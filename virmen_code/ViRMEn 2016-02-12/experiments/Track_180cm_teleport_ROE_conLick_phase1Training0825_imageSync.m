function code = Track_180cm_teleport_ROE_conLick_phase1Training0825_imageSync
%2000920. EBH test image synch TTL from scanbox, input into ai5. made
%imageSync global to save here
...
% Number of reward zones(whole track = 12)---------------------------------
vr.numRewZones=4; %set the desired number of reward zones
...
vr.lickThreshold = -0.085;% might need to adjust, unfortunately the clampex recording is not reliable, use licksensor_toy
...
global lickSensor islick roe timeROE %200920
vr.t=vr.t+1;
...
if sum(vr.rewZone(vr.t)==vr.legalRewZones)==1
    if islick && vr.position(2)>10 && sum(vr.reward(vr.trialNum==vr.trialNum(vr.t)))==0
        vr.isReward=1;
...
VR.imageSync=vr.imageSync;%200920. EBH
VR.reward=vr.reward;
VR.time=vr.time;
VR.lick=vr.lick;
VR.ypos=vr.ypos;
