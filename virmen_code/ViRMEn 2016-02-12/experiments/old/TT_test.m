...
% 12)---------------------------------
vr.gainFactor = 2/3;
vr.numRewZones=12; %set the desired number of reward zones
vr.rewardZone = 10*vr.gainFactor; % length of reward zone after currentGoal
...
vr.lickThreshold = -0.05;% might need to adjust, unfortunately the clampex recording is not reliable, use licksensor_toy
...
% box for monitoring lick V
vr.text(2).position = [-3.5 -1.0]; % lower-left corner of the screen
vr.text(2).size = 0.03; % letter size as fraction of the screen
vr.text(2).color = [1 0 0]; %
% end box by TT

% box for monitoring rewZone
vr.text(3).position = [-3.5 -1.2]; % lower-left corner of the screen
vr.text(3).size = 0.03; % letter size as fraction of the screen
vr.text(3).color = [1 0 0]; %
% end box by TT

% box for monitoring yPos
vr.text(4).position = [-3.5 -1.4]; % lower-left corner of the screen
vr.text(4).size = 0.03; % letter size as fraction of the screen
vr.text(4).color = [1 0 0]; %
% end box by TT
...
% lickVoltage monitor
vr.text(2).string = vr.lickVoltage(vr.t);
% monitor end.
...
vr.text(3).string = vr.rewZone(vr.t);
vr.text(3).color = [1 0 0];
...
if double(vr.keyPressed == 51) && vr.numRewZones<25 %ascii code for "3"
