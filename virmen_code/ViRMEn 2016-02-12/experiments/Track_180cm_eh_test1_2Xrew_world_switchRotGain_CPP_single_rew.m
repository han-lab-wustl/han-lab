function code = Track_180cm_eh_test1_2Xrew_world_switchRotGain_CPP_single_rew
% gives water rewards at interval when restricted to one room. 190614 EH
% modified original code so it doesn't track time in rooms
% made generic labels
...
% vr = initializeDAQ_CPP(vr);%orig from VR. Think V to rew is only diff
vr = initializeDAQ(vr);
...
vr.rewTimeA= 1885;
vr.rewTimeB= 1885;
vr.rewTime= 8815; %modify this for both rooms. 190614 EH
...
vr.elapse=0;%190614 EH
vr.Aelapse=0;
vr.Belapse=0;
vr.Astart=0;
vr.start=0;%190614 EH
vr.Bstart=0;
vr.Arewards=0;
vr.Brewards=0;
vr.rewards=0;%190614 EH
...
%     vr.time=vr.time+etime(vr.curr, vr.last);
    if vr.start(1)>0%190614 EH
        vr.elapse=etime(vr.curr, vr.start);
    else
        vr.start=vr.curr;        
    end

% 
if vr.elapse>vr.rewTime%190614 EH
%     disp('reward A');
    reward(vr,vr.timeSolenoid);
    vr.numRewards = vr.numRewards + 1;
    vr.rewards=vr.rewards+1;
