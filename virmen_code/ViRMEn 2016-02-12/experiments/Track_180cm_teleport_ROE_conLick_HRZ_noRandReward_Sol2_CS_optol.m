function code = Track_180cm_teleport_ROE_conLick_HRZ_noRandReward_Sol2_CS_optolongstim
...
vr.numinbetweenOpto = 0; %number of opto periods in between rewards
vr.numRewardOpto = 10; %number of perio reward opto
vr.optoDur = 5000; % number of miliseconds for opto duration
...
vr.timerSaved = [];
% vr.initialPeriod = 10;
vr.minDoubleInterval = 1; %number of rewards before next available double reward
vr.buffer = 20;
...
for kk = 1:vr.numRewardOpto
    ...
    vr.optoEvents = [ vr.optoEvents sum(vr.newInterval(1:newDoubleReward))+vr.initialWaitPeriod-0.5];% + 0.5*(newDoubleReward-1)*(newDoubleReward~=1)];
