function code = Track_270cm_lightworld_10cmRL_ZD_HRZ_LickLimit
...
vr.lickLimit = 10; % indicate how many licks to penalty here!
...
if vr.lick(1,vr.t) == 1 %display the detected lick
    vr.text(1).string = 'L';
    % zd edits
    % at every detected lick, check if the previous frame had no lick
    % if so, add to lick counter
    % if this counter is exceeded, the mouse will get a breif sec interval
    % and then be put in dark time (penalty). the lick limit can be
    % adjusted in the initial params
    if vr.eligible == 1 && vr.lick(1,vr.t-1) == 0
        vr.curnumLick = vr.curnumLick+1;
        if vr.curnumLick == vr.lickLimit
