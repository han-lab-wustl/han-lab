function code = Track_180cm_teleport_ROE_conLick_HRZ_ImagingONOFF
...
vr.currentWorld = 1; % start on the second world (ON/OFF world), change to 1 to start in the world with the usual rules
...
p = 0.0; % probability of having probe trial at the beginning
vr.lickThreshold = -0.015; % might need to adjust
vr.goal = 90; % HERE state the automatic reward location
vr.lickControl = 1; % HERE state the if lick is necessary to get reward
vr.rewardZone = 15*vr.gainFactor; % length of reward zone 
...
vr.automaticExperimentWorld = 1; % automatically change the current World after n = vr.automaticExperimentWorldNRewZones reward locations.
...
vr.worldONTime = 1; % amount of seconds the world stays on
...
if vr.currentWorld == 1
    vr.nW1Changes = 1; %how many times the world1 appeared. Set to 1 considering the experiment starts from W1.
    vr.nW2Changes = 0; %how many times the world2 appeared.
else
    vr.nW1Changes = 0; %how many times the world1 appeared. Set to 1 considering the experiment starts from W1.
    vr.nW2Changes = 1; %how many times the world2 appeared.
end
...
vr.stop = 1; % set displacement =0 between trials 
vr.timeOn = []; %time when the animal see the world
vr.timerON = 0; %timer to render the world visible is on
...
if vr.currentWorld == 1 %condition in the first world
...
else %condition in the second world
    if vr.pressedKeys(vr.t) == 48 && vr.pressedKeys(vr.t-1) ~= 48 %when the world turns on
        vr.timeOn = vr.time(1,vr.t);
        vr.stop = 0;
    end
    if vr.position(2)== vr.beginZone && vr.time(1,vr.t)>=vr.time2dark+((vr.darkTime-1)+vr.random_time) % variable length in dark
        vr.worlds{vr.currentWorld}.surface.visible(:) = true;
        ...
        vr.stop = 0;
        if vr.timerON == 1
            vr.timeOn = vr.time(1,vr.t); %time when the world turns on
        end
        vr.timerON = 0;
    end
    if vr.time(1,vr.t)>vr.timeOn+vr.worldONTime
        vr.worlds{vr.currentWorld}.surface.visible(:) = false;
        vr.worlds{1, 2}.backgroundColor = [0.22 0.22 0.22];
    end
%     if vr.worlds{vr.currentWorld}.surface.visible(:) == true %
%     possibility of preventing movements when the world 2 is on
%         vr.dp(1:4) = [0 0 0 0];
%     end
    if vr.worlds{vr.currentWorld}.surface.visible(:) == false
        if  vr.stop == 1 % prevent the VR from moving when the screen is black
            vr.dp(1:4) = [0 0 0 0];
            vr.worlds{1, 2}.backgroundColor = [0 0 0];
            vr.timerON = 1 ;%turn on the timer
        end
    end
end
