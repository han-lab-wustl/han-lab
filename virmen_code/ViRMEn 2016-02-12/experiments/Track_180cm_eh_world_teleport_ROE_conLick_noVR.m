function code = Track_180cm_eh_world_teleport_ROE_conLick
%new world, unidirectional run, reward at end, teleport to beginning
%200311 EBH.  modified to use contact lick data into daq
...
vr = initializeDAQ_low_view_V_conLick_ROE(vr);
...
vr.lickThreshold = -0.037;% might need to adjust
...
% if currentlick<vr.lickThreshold
%     play_mario_coin()
% end


% if vr.conLic<vr.lickThreshold
...
%         if double(vr.keyPressed) == 43  %ascii code for "+"
    ...
    test= inputSingleScan(vr.moveSession);
    testLick=test(3);
    
% if testLick < vr.lickThreshold
%         islick = 1;
%         sound(1)
%     else
%         islick = 0;
...
symbolYPosition = 2*(testLick-vr.trackMinY)/(vr.trackMaxY-vr.trackMinY) - 1;
