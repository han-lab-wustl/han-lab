...
if vr.random_initiation
    a=[{40:80} {80:120} {120:160}]; %defines the possible target zone EB 1007
...
if double(vr.keyPressed) == 45 || double(vr.keyPressed) == 95  %ascii code for "-/_"
    ...
    a=[{20:60} {70:110} {120:160}];%defines the regions, here we have three
    if sum(vr.changeRewLoc>=1)<vr.numMainRewZone
        ...
        checkIndx=find(vr.changeRewLoc > 0);
          if vr.changeRewLoc(1,checkIndx(1,sum(vr.changeRewLoc>=1)))-temp < 20
            temp=temp-randi([21,30]);
        elseif vr.changeRewLoc(1,checkIndx(1,sum(vr.changeRewLoc>=1)))-temp > -20
            temp=temp+randi([21,30]);
        end
    else
        temp = randi([50,150]);
...
trials=vr.trialNum;
prompt='type mouse name and date in the following format: MouseName_YY/MM/DD, use '' ';
f=inputdlg(prompt);
if exist('f','var')
save(['C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\threshold\lickThreshold' f{1,1} '.mat'],'lickThreshold')
save(['C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\time\time' f{1,1} '.mat'],'time')
save(['C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\lick\lick' f{1,1} '.mat'],'lick')
save(['C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\changeRewLoc\changeRewLoc' f{1,1} '.mat'],'changeRewLoc')
save(['C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\yposition\yposition' f{1,1} '.mat'],'yposition')
save(['C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\reward\reward' f{1,1} '.mat'],'reward')
save(['C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\lickVoltage\lickVoltage' f{1,1} '.mat'],'lickVoltage')
save(['C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\trials\trials' f{1,1} '.mat'],'trials')
end
