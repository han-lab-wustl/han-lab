function code = Track_180cm_teleport_ROE_conLick_HRZ_EBH1003ImagingTestEBGME134
...
vr.gainFactor = 0.3; % Gain in the track. The smaller, the longer the track. 1= usual gain. This value is multiplied by num rpm from ROE
p = 0.4;%probability of having probe trial at the beginning
vr.lickThreshold = -0.0280; % might need to adjust
...
vr.text(1).size = 0.01; % letter size as fraction of the screen
...
if  vr.time2EndCheck == 1
    if  (vr.time(1,vr.t)-vr.time2End)>=vr.endTime % chech the animal stayed at the end for the desired time
        ...
        vr.gainFactor = randi([3,7])/10; % VARIABLE GAIN!!!
