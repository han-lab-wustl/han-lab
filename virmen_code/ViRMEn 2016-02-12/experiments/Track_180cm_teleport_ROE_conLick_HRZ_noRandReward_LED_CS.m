function code = Track_180cm_teleport_ROE_conLick_HRZ_noRandReward_LED_CS
...
vr = initializeDAQ_low_view_V_conLick_ROE_no_imageSync(vr); %200920 EBH
def_ROE_VR2_rewardLED ;
...
negativeLEDp = 1; % probability that this session contains 1 LED but no reward trial.
...
vr.imageNumFrames = 50000;% scanbox number of frames to be recorded
vr.imageFrameRate = 31.25; % scanbox recording frames per second
vr.numDoubleRewards = 4; %number of double rewards per session
vr.LED2rewDelay = 1;%seconds after the LED turns on that the reward is given.
vr.LEDpulsewidth = 1; %seconnds that the LED is on for;
vr.initialTime = 0; %seconds before giving rewards
...
vr.manualDouble = 0; % boolean in case we decide to manually give a double reward
...
vr.newInterval = [];
i = 1;
while sum(vr.newInterval)<vr.timeImaging-vr.initialPeriod-vr.buffer
...
if rand < negativeLEDp
% vr.negativeCS(randi([1,length(vr.newInterval)])) = 1;
vr.negativeCS(1) = 1;
end
...
if vr.LEDTimer <= vr.LED2rewDelay-vr.LEDpulsewidth
    writeDigitalPin(vr.arduino,vr.chLED,0)
end
if vr.LEDTimer <= 0
        vr.saved = vr.timer;
    if ~isempty(vr.negativeCS(vr.rewardIdx) == 1)
        if vr.doubleReward(vr.rewardIdx) == 1 || vr.manualDouble == 1
            vr.isReward = 2;
...
vr.timer = toc;
% if vr.timer-vr.saved>=vr.newInterval(vr.rewardIdx) && vr.timer > vr.initialTime+vr.newInterval(1) && vr.notDone && vr.LEDcount == 0 % afer 2 mins in the dark, start giving random rewards
%     %     vr.saved = vr.timer;
%     disp(vr.saved)
%     %     if vr.doubleReward(vr.rewardIdx) == 1
%     %         vr.isReward = 2;
%     %         reward_double(vr,vr.timeSolenoid);
%     %     else
%     %         vr.isReward = 1;
...
%     %     end
%     vr.isReward=0.5;
%     writeDigitalPin(vr.arduino,vr.chLED,1)
%     vr.LEDcount = 1;
%     vr.LEDTimer = vr.LED2rewDelay;
% end
...
if exist('name_date_vr','var')
    save(['C:\Users\Han Lab\Documents\MATLAB\tillTheEnd_variables\VR_data\' name_date_vr '.mat'],'VR') 
    load('C:\Users\Han Lab\Documents\MATLAB\tillTheEnd_variables\summary','name_date');
    name_date{size(name_date,1)+1,1}=name_date_vr;
    save('C:\Users\Han Lab\Documents\MATLAB\tillTheEnd_variables\summary','name_date')
end
