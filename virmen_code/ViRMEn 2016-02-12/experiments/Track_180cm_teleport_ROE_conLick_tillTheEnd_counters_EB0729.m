function code = Track_180cm_teleport_ROE_conLick_tillTheEnd_counters_EB0729
...
if vr.random_initiation
    a=[{40:80} {81:120} {121:160}]; %defines the possible target zone EB 1007
...
vr.trialNum=3; %count num of trials, if lower than three means the reward location has just changed
vr.lickVoltage=[];
vr.time=[];
vr.roe=[];
vr.timeROE=[];
...
vr.lickVoltage = [vr.lickVoltage lickSensor];  %state from where we are getting the lick
%%signal remember to update signal file
vr.time = [vr.time toc];
vr.roe =[vr.roe roe];% Read current count from the quadrature rotary encoder.
vr.timeROE=[vr.timeROE timeROE]; % Time elasped in seconds since Arduino server starts running (double)
vr.lick = [vr.lick islick];
vr.ypos = [vr.ypos vr.position(2)];
vr.changeRewLoc=[vr.changeRewLoc 0];
vr.trialNum=[vr.trialNum vr.trialNum(length(vr.trialNum))];

if vr.trialNum(end)==0
    vr.isChange=0;
end
...
if (vr.positionsuccess && vr.licksuccess == 1) && (vr.sametrial == 0 && vr.firsttimecheck == 0 &&  vr.trialNum(end)>2) % if you don't want lick control then just set vr.numbertolick = 0 in initiation
...
if vr.rewardTimer > vr.rewardTime && (vr.endtimecheck == 1) && vr.rewardTimer <= (vr.rewardTime+vr.rewarddarkTime) && (vr.position(2)>= vr.topTarget)
    ...
    vr.dp(1:4) = [0 0 0 0];
    vr.sametrial = 0;
    vr.trialNum=[vr.trialNum vr.trialNum+1];
...
if  vr.isChange==1 && vr.position(2)==vr.beginZone
    ...
    vr.changeRewLoc=[vr.changeRewLoc temp];
...
if vr.isReward
    ...
    if vr.isReward==1
        vr.reward = [vr.reward vr.isReward];
    else
...
name_date_vr=[mouse_name{1,1} '_' date{1,1}];
if exist('mouse_name','var') && exist('date','var')
    save(['C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\threshold\lickThreshold' name_date_vr '.mat'],'lickThreshold')
    save(['C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\time\time' name_date_vr '.mat'],'time')
    save(['C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\lick\lick' name_date_vr '.mat'],'lick')
    save(['C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\changeRewLoc\changeRewLoc' name_date_vr '.mat'],'changeRewLoc')
    save(['C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\yposition\yposition' name_date_vr '.mat'],'ypos')
    save(['C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\reward\reward' name_date_vr '.mat'],'reward')
    save(['C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\lickVoltage\lickVoltage' name_date_vr '.mat'],'lickVoltage')
    save(['C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\trials\trials' name_date_vr '.mat'],'trials')
    save(['C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\roe\ROE' name_date_vr '.mat'],'ROE')
    save(['C:\Users\imaging_VR\Documents\MATLAB\tillTheEnd_variables\roe\timeROE' name_date_vr '.mat'],'timeROE')
