function code = Track_180cm_eh_world_teleport_ROE_conLick_control2_noVR
...
% catch
    vr.lickThreshold = -0.84 ;% might need to adjust, unfortunately the clampex recording is not reliable, use licksensor_toy
% end
vr.gamemode = 1; % decide if the lick outside "reward zone" counts
vr.random_initiation = 0; % randomly initiate reward for world 2
vr.allGoals = [100 50]; %set default goal [world 1 world 2]. 
if vr.random_initiation
    vr.allGoals(2) = randi([50,150]);%world 2 random [min max]
end
...
% vr.currentGoal = 100;
vr.rewardZone = 5; % length of reward zone after currentGoal
...
vr.rewardTime = 5;%time after reward that animal still in VR?
...
vr.worlds{vr.currentWorld}.surface.visible(:) = false;%make world  invisible  
global islick
% add lick checking module
% logic: everytime it gives n licks, check the time difference between the nththe time elapsed
...
% shrinking
if vr.totaltrialcount> vr.trialtoaccuracycheckpoint && vr.gamemode == 1 %ignores first trials
    % check accuracy to adjust reward zone size
    if vr.accuracy<50 && vr.rewardZone<180 % don't make it too large 
        vr.rewardZone = vr.rewardZone+5;%increases 5cm when voerall accuracy low. may want to change to shorter window
        disp('===')
    ...
    elseif vr.accuracy>80 && vr.rewardZone>10%keeps shrinking if accuracy >80%
        vr.rewardZone = vr.rewardZone-5;
        disp('===')
        fprintf('Rew9ard Zone = %i units \n',vr.rewardZone)
        fprintf('Total Trial = %i, accuracy = %2.1f %% \n',vr.totaltrialcount,vr.accuracy)
        vr.trialtoaccuracycheckpoint = vr.totaltrialcount+10;
    end
end
...
%textboxes in lower left
vr.text(1).string = num2str(vr.lickThreshold); %not all digits visible
% num2str(vr.lickThreshold)
% vr.text(1).string = num2str(vr.currentGoal,'%3f');
% vr.text(1).string = num2str(vr.rewardZone,'%4.2f');
% vr.text(1).string =fprintf('Reward Zone = %i units \n',vr.rewardZone);
