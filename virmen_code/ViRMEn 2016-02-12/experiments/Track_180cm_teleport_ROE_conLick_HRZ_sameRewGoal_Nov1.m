function code = Track_180cm_teleport_ROE_conLick_HRZ_sameRewGoal_Nov1
...
vr.goal = 144; % HERE state the automatic reward location% 75cm for fam1 EB 0625 % 144cm for nov1 EB 0627
...
if vr.worlds{vr.currentWorld}.surface.visible(:) == false % prevent the VR from moving when the screen is black
    ...
    vr.worlds{1, 2}.backgroundColor = [0 0 0]; % dark at the beginning
    if ~isempty(check)
        if vr.currentWorld == 2 && vr.worldCurrent(check) ~= 1 %EB 0627
