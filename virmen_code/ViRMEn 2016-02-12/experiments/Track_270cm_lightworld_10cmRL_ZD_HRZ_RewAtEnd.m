function code = Track_270cm_lightworld_10cmRL_ZD_HRZ_RewAtEnd
% 200610. EH modify to use contact lick sensor instead of optical
% 8/8/24 - zd made edits with gerardo to deliver reward at the end
...
% end
% delivers solenoid sound (CS in rig1)s and sets the cs condition to 1 to
% deliver reward at the end
...
if vr.trialNum(1,vr.t)>vr.numProbes-1 && ((vr.position(2)>(vr.goal-(vr.rewardZone/2)) && vr.position(2)<(vr.goal+(vr.rewardZone/2))) || (vr.posControl==0))
    if (vr.eligible==1) || (vr.countControl==0)
        ...
        elseif vr.lickControl==1 && vr.lick(1,vr.t) == 1
            ...
            vr.eligible=0;
            vr.curRewEpoch = vr.curRewEpoch + 1;
            % when animal licks in the right spot, do not dispense reward
            %             reward(vr,vr.timeSolenoid);
        end
    else
...
% running into wall/entering dark time
if  vr.collision && vr.position(2)>vr.topTarget  && vr.time2EndCheck == 0 %vr.position(2)>= vr.topTarget
    ...
    % at the end of the track (when the timer to enter dark time is up),
    % deliver reward if the animal licks in rew zone
    if vr.CSsuccess == 1
        vr.isReward = 1;
        reward(vr,vr.timeSolenoid)
        vr.reward(1,vr.t) = vr.isReward;
    end
    vr.CSsuccess = 0; % reset condition
end
