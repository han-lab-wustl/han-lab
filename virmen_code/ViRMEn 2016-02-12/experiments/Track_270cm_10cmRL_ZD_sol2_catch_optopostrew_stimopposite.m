function code = Track_270cm_10cmRL_ZD_sol2_catch_optopostrew_stimopposite
...
vr.minsuccessCatch = 7;
vr.maxsuccessCatch = 14;
...
vr.zones = [135 135 135]; % single rew zone
%    a=[{67:86} {101:120} {135:154}]; % defines the possible target zone
vr.stimZone=rem((vr.zones(1)-(vr.rewardZone/2)+90),180);
if vr.stimZone<15
    vr.stimZone = 15;
elseif vr.stimZone>160
    vr.stimZone=160;
end
...
if vr.optoIson == 0 % ZD: stim when animal is at the opposite side of track
     if vr.trialNum(1,vr.t)>vr.numProbes-1 && (vr.position(2)>vr.stimZone) && sum(vr.optoEp == sum(vr.changeRewLoc~=0))>0  && ~rem(vr.trialNum(1,vr.t),10)==0
