function code = Track_180cm_teleport_ROE_conLick_RandReward_Sol2_CS_Surprise
...
% negativeLEDp = 0; % probability that this session contains 1 LED but no reward trial.
vr.lickThreshold = -0.086; % Voltage threshold on Lickometer. Lower than this is considered a lick. lmight need to adjust
vr.goal = 90; % HERE state the automatic reward location
vr.lickControl = 1; % HERE state the if lick is necessary to get reward
vr.rewardZone = 5*vr.gainFactor; % length of reward zone 
...
%Random Rewards variables
vr.imageNumFrames = 40000;% scanbox number of frames to be recorded
vr.imageFrameRate = 31.25; % scanbox recording frames per second
vr.numDoubleRewards = 0; %number of double rewards per session
vr.numNegativeCSs = 0; %number of CS without reward
vr.numNegativeRewards = 2; %number of Rewards without CS
vr.minNegativeRewardBuffer = 8;%minimum seconds between random CS-US pair to insert an unpaired US
vr.initialWaitPeriod = 120; %seconds before delivering automatic rewards;
vr.minIRI = 15; %minimum time between rewards automatic
vr.maxIRI = 45; %maximum time between rewards automatic
...
vr.timerSaved = [];
% vr.initialPeriod = 10;
vr.minDoubleInterval = 3; %number of rewards before next available double reward
vr.buffer = 20;
vr.timeImaging = vr.imageNumFrames/vr.imageFrameRate;
vr.newInterval = [];
i = 1;
while sum(vr.newInterval)<vr.timeImaging-vr.initialWaitPeriod-vr.buffer
vr.newInterval(i) = rand()*(vr.maxIRI-vr.minIRI)+vr.minIRI;
i = i +1;
end
vr.newInterval(length(vr.newInterval)) = [];
vr.rewardIdx = 1;
vr.notDone = 1;

vr.doubleReward = zeros(size(vr.newInterval));
availableIdx = 1:length(vr.newInterval);
if vr.numDoubleRewards>0
for kk = 1:vr.numDoubleRewards
    newDoubleReward = availableIdx(randi(length(availableIdx)));
    vr.doubleReward(newDoubleReward) = 1;
    
    unavailableIdx = newDoubleReward-vr.minDoubleInterval...
        :newDoubleReward+vr.minDoubleInterval;   
    [~,ia,ib] = intersect(unavailableIdx,availableIdx);
    availableIdx(ib) = [];
    
end
end
vr.negativeCS = zeros(size(vr.newInterval));
if vr.numNegativeCSs > 0 
    for kk = 1:vr.numNegativeCSs
        newDoubleReward = availableIdx(randi(length(availableIdx)));
        vr.negativeCS(newDoubleReward) = 1;
...
vr.negativeRewardTimePoints = zeros(1,vr.numNegativeRewards);
if vr.numNegativeRewards > 0 
    
    for kk = 1:vr.numNegativeRewards
        newDoubleReward = availableIdx(randi(length(availableIdx)-1));
        newtime = sum(vr.newInterval(1:newDoubleReward))+vr.initialWaitPeriod... %total time till CS-US reward chose + initial waiting period +...
            +rand*(vr.newInterval(newDoubleReward+1)-2*vr.minNegativeRewardBuffer)+vr.minNegativeRewardBuffer; %random amount of time after CS-US and before next CS-US with a 5 second buffer.
        vr.negativeRewardTimePoints(kk) = newtime;
        unavailableIdx = newDoubleReward-vr.minDoubleInterval...
            :newDoubleReward+vr.minDoubleInterval;
        [~,ia,ib] = intersect(unavailableIdx,availableIdx);
        availableIdx(ib) = [];
...
vr.negativeRewardTimePoints=sort(vr.negativeRewardTimePoints);
vr.savenegRewardTime = vr.negativeRewardTimePoints;
...
%setup for basic VR things
if vr.position(2)<5 && vr.velocity(2)>0 %ensure the mouse doesn't get stuck at the beginning
    vr.collision = 0; % one way wall
    vr.dp(2) = vr.velocity(2)*vr.dt;
end
global lickSensor roe timeROE  imageSync % import from movement function some necessary daq signals
vr.t=vr.t+1;
vr.imageSync(1,vr.t)=imageSync; %store imaging TTL for saving
...
vr.pressedKeys(1,vr.t)=vr.keyPressed; %save any button presses you make
vr.worldCurrent(1,vr.t)=vr.currentWorld ;
vr.isReward=0; %ensure that the start of every frame has 0 rewards and must go through each condition frame by frame
...
if etime(clock,vr.PastReward) >= vr.LED2rewDelay && vr.LEDcount
    ...
    if vr.automaticReward == 1
    if vr.rewardIdx<length(vr.newInterval)
        vr.rewardIdx = vr.rewardIdx + 1;
    else
        vr.notDone = 0;
    end
    vr.automaticReward = 0;
    end
    vr.LEDcount = 0;
    vr.LEDTimer = vr.LED2rewDelay;
end
...
%%random reward automatic timers

vr.timer = toc;
if vr.timer-vr.saved>=vr.newInterval(vr.rewardIdx) && vr.timer > vr.initialWaitPeriod+vr.newInterval(1) && vr.notDone && ~vr.LEDcount % afer 2 mins in the dark, start giving random rewards
    if vr.negativeCS(vr.rewardIdx) == 1 %if current random reward is a negative CS
        vr.negativeCSQueue = 1;
    elseif vr.doubleReward(vr.rewardIdx) == 1 %if current random rewards is a double
        
        vr.doubleQueue = 1;
    else %if current random reward is a single
        vr.singleQueue = 1;
    end
    vr.automaticReward = 1; %assert that we are still giving random rewards (the recording period isn't over)
    vr.saved = vr.timer;
end

% negative reward section (US without a CS)
if ~isempty(vr.negativeRewardTimePoints)
    if toc > vr.negativeRewardTimePoints(1)
        vr.negativeRewardTimePoints(1) = [];
        vr.isReward = 1;
        reward(vr,vr.timeSolenoid)
    end
end

vr.reward(1,vr.t) = vr.isReward; %save the final decision for this frame (did a reward happen or not?)
...
%     end
% end
%set next reward to be a negative CS trial
if double(vr.keyPressed == 126)
    if vr.rewardIdx < length(vr.negativeCS)
    vr.negativeCS(vr.rewardIdx+1) = 1;
    end
end
...
%give a double reward
if double(vr.keyPressed == 332)
vr.doubleQueue = 1;
end
% give a negative CS

if double(vr.keyPressed == 333)
vr.negativeCSQueue = 1;
end
...
if double(vr.keyPressed == 55) && vr.rewardZone>=3 %ascii code for "7"
 disp('decrease')
 disp(num2str(vr.rewardZone))
 vr.rewardZone =  vr.rewardZone - 1*vr.gainFactor ;
 disp(num2str(vr.rewardZone))
end
...
if vr.worlds{vr.currentWorld}.surface.visible(:) == false % prevent the VR from moving when the screen is black
    vr.dp(1:4) = [0 0 0 0];
   vr.worlds{1, 2}.backgroundColor = [0 0 0]; % change the VR background color
else 
   vr.worlds{1, 2}.backgroundColor = [0.22 0.22 0.22];% change the VR background color
end
...
VR.wOff = vr.wOff ;
vr.gainFactor = 1; % Gain in the track. The smaller, the longer the track. 1= usual gain. This value is multiplied by num rpm from ROE
VR.newInterval = vr.newInterval; %210315 GM ZD
VR.doubleReward = vr.doubleReward;
VR.savenegRewardTime = vr.savenegRewardTime;
...
settings.imageNumFrames = vr.imageNumFrames; %number framesw recorded
settings.imageFrameRate = vr.imageFrameRate; %imaging frame rate
% Timers - seconds
