function code = Track_180cm_teleport_ROE_conLick_HRZ_EBH0929ImagingTest
...
scalingFACTOR = 0.5; % Gain in the track. The smaller, the longer the track. 1= usual gain. This value is multiplied by num rpm from ROE
...
vr.beginZone = 3; % value 1 does not allow movements
...
global lickSensor roe timeROE scalingFACTOR imageSync
vr.t=vr.t+1;
...
isReward1=vr.position(2)>vr.goal-(vr.rewardZone/2);
isReward2=vr.position(2)<vr.goal+(vr.rewardZone/2);
rew=floor((isReward2+isReward1)/2);
rew_el=floor((rew+vr.eligible)/2);


if floor((rew_el+vr.lick(1,vr.t))/2) == 1
    vr.reward(1,vr.t)=1;
    vr.eligible=0;
...
if  vr.time2EndCheck == 1
    if  (vr.time(1,vr.t)-vr.time2End)>=vr.endTime % chech the animal stayed at the end for the desired time
        ...
        if  vr.changeCurrentWorld == 1 && vr.trialNum(1,vr.t)>vr.numProbes-1
            vr.currentWorld = 1;
            vr.nW1Changes = vr.nW1Changes+1;
        elseif vr.changeCurrentWorld == 2 && vr.trialNum(1,vr.t)>vr.numProbes-1
            vr.currentWorld = 2;
...
VR.ROE=vr.roe(1,1:vr.t);
VR.lickThreshold = vr.lickThreshold;
VR.reward=vr.reward(1,1:vr.t);
VR.time=vr.time(1,1:vr.t);
VR.lick=vr.lick(1,1:vr.t);
VR.ypos=vr.ypos(1,1:vr.t);
VR.lickVoltage=vr.lickVoltage(1,1:vr.t);
VR.trialNum=vr.trialNum(1,1:vr.t);
VR.timeROE=vr.timeROE(1,1:vr.t);
VR.changeRewLoc=vr.changeRewLoc(1,1:vr.t);
VR.pressedKeys=vr.pressedKeys(1,1:vr.t);
VR.world=vr.worldCurrent(1,1:vr.t);
VR.imageSync=vr.imageSync(1,1:vr.t);%2000920. EBH
VR.scalingFACTOR=scalingFACTOR;
VR.wOff = vr.wOff(1,1:vr.t);
