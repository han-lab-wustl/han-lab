function code = Track_180cm_teleport_ROE_conLick_HRZ_1stdayNEWCS4slag_Sol2_CS
...
vr.LED2rewDelay = 4;%seconds after the solen oid 2 turns on that the reward is given.
...
vr.initialCSUStime = 7*60; % seconds a(after/not including 2 min buffer) thatCS still gives US. after, CS no US
% Timers - seconds
...
vr.negativeCS = ones(size(vr.newInterval));
vr.negativeCS(find(cumsum(vr.newInterval)<vr.initialCSUStime,1,'last'):end) = 0;
