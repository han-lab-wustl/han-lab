function code = Track_180cm_teleport_ROE_conLick_tillTheEnd_counters_EB0825
...
if (vr.positionsuccess && vr.licksuccess == 1) && (vr.sametrial == 0 &&  vr.trialNum(end)>2) % if you don't want lick control then just set vr.numbertolick = 0 in initiation
...
% automatically change the reward zone
if vr.trialNum(vr.t)>15 
    lastRewLoc=find(vr.changeRewLoc>1,1,'last');
    numRew=sum(vr.reward(lastRewLoc:vr.t));
    if numRew>randi(20:25)
    vr.isChange=1;
    end
end
