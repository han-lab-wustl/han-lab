function code = Track_270cm_10cmRL_ZD_sol2_catch_optodarktime
...
vr.minsuccessCatch = 3;
vr.maxsuccessCatch = 10;
vr.curCatchInterval = 0;
vr.pastCatch = 0;

% Timers - seconds
vr.endTime = 0.2 ;%vr.rewardTime = 1;%time after reaching the end of the track that animal spend in VR
vr.darkTime = 4 ;%vr.rewarddarkTime = 2; % amount of seconds after vr.rewardtime that you are in the dark before reappearing at the start, now it goes from 3:5s
% zd extended dark time
...
vr.zones = [114 114 114]; % single rew zone
...
%     if (sum(double(vr.worlds{vr.currentWorld}.surface.visible(:))) > 0) && (rem(vr.trialNum(1,vr.t),2)==1)
...
%      if vr.trialNum(1,vr.t)>vr.numProbes-1 && ((vr.position(2)>(vr.goal+(vr.rewardZone/2))) || vr.eligible == 0) && sum(vr.optoEp == sum(vr.changeRewLoc~=0))>0  && rem(vr.trialNum(1,vr.t),2)==1
...
if vr.optoIson == 0 % ZD: only in dark time and extend dark time
    if sum(vr.optoEp == sum(vr.changeRewLoc~=0))>0 && vr.trialNum(1,vr.t)>vr.numProbes-1 && rem(vr.trialNum(1,vr.t),2)==1
        vr.optoIson = 1; %prevents repeated stim
        write(vr.optodeviceObj,[1 2000],'uint16')
        vr.optoTrigger = [vr.optoTrigger {[vr.t,1,2000]}];
    end
end
