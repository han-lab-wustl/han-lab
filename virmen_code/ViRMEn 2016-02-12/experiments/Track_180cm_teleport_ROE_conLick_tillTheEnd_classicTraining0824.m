function code = Track_180cm_teleport_ROE_conLick_tillTheEnd_classicTraining0824
...
if  vr.isChange==1 && vr.position(2)==vr.beginZone
    if sum(vr.changeRewLoc>=1)<=vr.numMainRewZone-1
        ...
        tempmindist = min(abs(vr.changeRewLoc(find(vr.changeRewLoc,1,'last'))-a{1,indx}));
        if tempmindist <= 30
            if min((vr.changeRewLoc(find(vr.changeRewLoc,1,'last'))-a{1,indx})) >= 0
                temp = randi([a{1,indx}(1),a{1,indx}(length(a{1,indx}))-(30-tempmindist)]);
            else
                temp = randi([a{1,indx}(1)+(30-tempmindist),a{1,indx}(lenght(a{1,indx}))]);
            end
        else
...
%     diff_rewZ=abs(vr.changeRewLoc(1,checkIndx(1,sum(vr.changeRewLoc>=1)))-temp); % previous reward location minus new reward location
%     if diff_rewZ < 30
%         if sum(vr.changeRewLoc>=1)<=vr.numMainRewZone-1
%             a=[{60:80} {81:120} {121:160}];%defines the regions, here we have three
