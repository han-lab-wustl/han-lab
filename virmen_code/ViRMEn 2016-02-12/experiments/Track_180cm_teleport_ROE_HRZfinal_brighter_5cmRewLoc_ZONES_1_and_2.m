function code = Track_180cm_teleport_ROE_HRZfinal_brighter_5cmRewLoc_ZONES_1_and_2
...
vr.i=zeros(1,300);
for i=1:3:300 %CHANGED TO ONLY FIRST TWO REWARD ZONES 8/31
...
while sum(corr>0)>0 % until there are no more consecutive zones
    for i=1:size(corr,2)
        vr.i(corr(i)+1:corr(i)+2)=nan; % define the triplette that has to be deleted 
    end
