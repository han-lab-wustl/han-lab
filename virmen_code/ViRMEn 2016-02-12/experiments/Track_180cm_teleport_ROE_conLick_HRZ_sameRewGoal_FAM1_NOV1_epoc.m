function code = Track_180cm_teleport_ROE_conLick_HRZ_sameRewGoal_FAM1_NOV1_epoc
...
vr.goal = 75; % HERE state the automatic reward location% 75cm for fam1 EB 0625
vr.choosenLoc = 0; % 2; 3; 1; 3; 2 Note: set to 0 to use vr.goal
...
vr.random_initiation = 0; % randomly initiate reward zone
...
vr.automaticExperimentWorld2NRewZones = 1; %how many reward zone in the world 2
...
vr.worldgoal = [75 144]; %goal locations for world 1 followed by world 2 GM 7/21
...
vr.minSuccessfulTrial = 22; %22 
vr.maxSuccessfulTrial = 27; %27
...
vr.currentWorld = randi(2); % default world at start - we only have 2 worlds in this esperiment %changed to random either 1 or 2 GM 7/21
...
elseif vr.choosenLoc ~=0
    a=[{67:86} {101:120} {135:154}]; % defines the possible target zone
    vr.goal = randi([min(a{vr.choosenLoc}),max(a{vr.choosenLoc})]);
end
...
vr.changeRewLoc=vr.worldgoal(vr.currentWorld); %reward location
vr.time=[];
vr.lick=[];
vr.ypos=[];
vr.reward=[];
...
vr.goal = vr.worldgoal(vr.currentWorld);
...
if vr.currentWorld == 2 && sum(vr.pressedKeys == 48) > 0
...
%     temp = a{1,indx}(rewZoneIdx(randi(length(rewZoneIdx)))); %randomly choose one of them
    vr.goal = vr.goal; %set the new reward zone
    vr.isChange=0;
    disp('===')
    ...
    if vr.currentWorld ==1 
          vr.changeRewLoc(1,vr.t)=vr.worldgoal(2);
    else
    vr.changeRewLoc(1,vr.t)=vr.worldgoal(1);
    end
end
...
if vr.worlds{vr.currentWorld}.surface.visible(:) == false % prevent the VR from moving when the screen is black
    ...
    if ~isempty(check) % if the world starts in world 2 and remapping is occurring we need to check that the first trial is fine GM 7/21
