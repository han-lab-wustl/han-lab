function code = Track_270cm_10cmRL_ZD_sol2_catch_optopostrew_4s
...
vr.zones = [86 86 86]; % single rew zone
...
if vr.optoIson == 0 % ZD: only post rew opto
     if vr.trialNum(1,vr.t)>vr.numProbes-1 && ((vr.position(2)>(vr.goal+(vr.rewardZone/2))) || vr.eligible == 0) && sum(vr.optoEp == sum(vr.changeRewLoc~=0))>0  && rem(vr.trialNum(1,vr.t),2)==1
        ...
        write(vr.optodeviceObj,[1 4000],'uint16')
        vr.optoTrigger = [vr.optoTrigger {[vr.t,1,4000]}];        
    end
end
