function code = Track_180cm_teleport_ROE_conLick_HRZ_RandReward_Sol2_CS_opto
...
vr.lickThreshold = -0.062; % might need to adjust
...
vr.numDoubleRewards = 2; %number of double rewards per session
vr.numNegativeCSs = 0;
vr.numNegativeRewards = 0;
...
vr.mininBetweenOptoBuffer = 8;%minimum seconds between random CS-US pair to insert an unpaired US
vr.numinbetweenOpto = 4; %number of opto periods in between rewards
vr.numRewardOpto = 4; %number of perio reward opto
vr.optoDur = 4700; % number of miliseconds for opto duration
vr.opto2cs = 2; %seconds before cs that opto turns on
vr.optoCSrandbuffer = 2; %minimum number of rewards between any periCS opto and any Random Opto
...
vr.timerSaved = [];
% vr.initialPeriod = 10;
vr.minDoubleInterval = 2; %number of rewards before next available double reward
vr.buffer = 20;
...
% vr.doubleReward = zeros(size(vr.newInterval));
% availableIdx = 1:length(vr.newInterval);
if vr.numRewardOpto>0 % adding peri reward opto events
for kk = 1:vr.numRewardOpto
    newDoubleReward = availableIdx(randi(length(availableIdx)));
   
    vr.optoEvents = [ vr.optoEvents sum(vr.newInterval(1:newDoubleReward))+vr.initialWaitPeriod-vr.opto2cs];% + 0.5*(newDoubleReward-1)*(newDoubleReward~=1)];
...
if vr.numinbetweenOpto > 0 %adding in between opto
    
    for kk = 1:vr.numinbetweenOpto
        ...
        newtime = sum(vr.newInterval(1:newDoubleReward))+vr.initialWaitPeriod... %total time till CS-US reward chose + initial waiting period +...
            +rand*(vr.newInterval(newDoubleReward+1)-2*vr.mininBetweenOptoBuffer)+vr.mininBetweenOptoBuffer; %random amount of time after CS-US and before next CS-US with a 5 second buffer.
        vr.optoEvents = [ vr.optoEvents newtime];
...
vr.optoEvents = sort(vr.optoEvents);
vr.optoEventssave = [];
...
%%Quing Rewards to after CS-US pair
if vr.singleQueue == 1
    if ~vr.LEDcount
        vr.isReward=0.5;
        if vr.DoubleFollowUp ~= 1
            vr.optotimeCorrection = vr.optotimeCorrection+0.0054;
...
if vr.timer-vr.saved>=vr.newInterval(vr.rewardIdx) && vr.timer > vr.initialWaitPeriod+vr.newInterval(1) && vr.notDone && ~vr.LEDcount % afer 2 mins in the dark, start giving random rewards
        vr.saved = vr.timer;
%     disp(vr.saved)
        if vr.negativeCS(vr.rewardIdx) == 1
            vr.negativeCSQueue = 1;
        elseif vr.doubleReward(vr.rewardIdx) == 1
%     %         vr.isReward = 2;
%     %         reward_double(vr,vr.timeSolenoid);
            vr.doubleQueue = 1;
        else
...
%     vr.LEDcount = 1;
%     vr.LEDTimer = vr.LED2rewDelay;
    vr.automaticReward = 1;
end

if ~isempty(vr.optoEvents)
if vr.timer > vr.optoEvents(1)+vr.optotimeCorrection
    write(vr.optodeviceObj,[1 vr.optoDur],'uint16')
    vr.optoEventssave = [vr.optoEventssave vr.t];
    vr.optoEvents(1) = [];
end
end
% 
...
VR.optoEventIdx = vr.optoEventssave;
