function code = Track_180cm_teleport_ROE_HRZ_GM_no_isync_LED_CS_flex
% 200610. EH modify to use contact lick sensor instead of optical
%220125 GM removied imagesync for VR2
%220131 GM added LED functions
%220309 GM added flexible buttons for setting next trial to 
%LED (no reward) and also a random chance of getting a trial with LED (no reward)
...
vr.lickThreshold = 1;%-0.086; % might need to adjust
...
vr.LEDpulsewidth = 0.2; %seconnds that the LED is on for;
minsuccessLEDtrials = 1;
maxsuccessLEDtrials = 1;
% Timers - seconds
...
vr.negativeCS = []; % list of trials that will present LED but not reward
...
if rand < negativeLEDp
    vr.negativeCS = [vr.negativeCS randi([minsuccessLEDtrials,maxsuccessLEDtrials])];
end
vr.cumtrialNum = [];
vr.cumtrialNum(1) = 1;
vr.worldCurrent=[];
...
vr.cumtrialNum(1,vr.t) = vr.cumtrialNum(vr.t-1);
vr.reward(1,vr.t)=0;
...
if vr.LEDTimer <= 0
    if ~isempty(find(vr.negativeCS == sum(vr.reward)))
    vr.isReward = 1;
...
% Next Trial LED but no Reward
if double(vr.keyPressed == 126)
    vr.negativeCS = [vr.negativeCS sum(vr.reward)+1];
end
...
if  vr.time2EndCheck == 1
    if  (vr.time(1,vr.t)-vr.time2End)>=vr.endTime % chech the animal stayed at the end for the desired time
        ...
        vr.cumtrialNum(1,vr.t) = vr.cumtrialNum(1,vr.t)+1;
