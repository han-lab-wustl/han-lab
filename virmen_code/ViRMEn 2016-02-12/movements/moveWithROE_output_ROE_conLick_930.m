...
function [movement,movementType] = moveWithROE_output_ROE_conLick_930(vr)
global islick
...
if strcmp(vr.movementType, 'displacement')
    if vr.count-roe < -60      %added a condition to prevent a large jump in the displacement as an artefact
        roe = vr.count-0.9*(vr.count-roe);
    else
        roe=vr.count;
        timeROE=vr.timeROE;
    end
